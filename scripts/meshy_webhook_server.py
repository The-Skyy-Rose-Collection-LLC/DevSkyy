...
class MeshyWebhookHandler(BaseHTTPRequestHandler):
    ...
    def handle_success(self, data: dict):
        ...
        try:
            urllib.request.urlretrieve(glb_url, str(output_path))  # nosec B310 — URL from controlled API response, not user input
                ...
                if tex_type:
                    for name, url in tex.items():
                        if url and url.startswith("http"):
                            ...
                            urllib.request.urlretrieve(url, str(tex_path))  # nosec B310 — URL from controlled API response, not user input
