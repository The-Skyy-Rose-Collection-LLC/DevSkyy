...
def verify_generate(
    ...
    from nano_banana.dna_prompts import build_prompt_from_dna, load_dna
    from nano_banana.tournament import pick_winner, run_tournament
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        for i, cand in enumerate(candidates):
            ...
            result = run_tournament(
                clients, source_path, PROJECT_ROOT / cand.path, dna, passing_threshold=98.0
            )
            ...
            log.info(
                ...
                "PASS" if result.passed_98 else "fail",
            )
        ...
        log.info(
            "Cycle %d winner: %.1f — %s",
            cycle,
            cycle_winner.aggregate_score,
            cycle_winner.candidate_path,
        )
        ...
        if cycle < max_cycles:
            ...
            feedback_metrics = {
                "issues": {"list": cycle_winner.top_issues, "fixes": cycle_winner.all_fixes}
            }
    ...
    if best_result and not best_result.passed_98:
        from nano_banana.composite_fallback import (
            hybrid_composite_from_dna,
            should_use_composite_fallback,
        )
