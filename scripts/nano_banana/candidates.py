...
def _save_candidate(image_bytes: bytes, sku: str, view: str, engine: str, model: str, attempt: int = 1, feedback: bool = False) -> Candidate:
...
def generate_candidates(
    ...
    """
    from nano_banana.generate import generate_gemini, generate_gpt, GEMINI_PRO, GPT_IMAGE_MODEL
    ...
    for i in range(n):
        ...
        log.info("[%s/%s] candidate %d/%d via %s (attempt %d)", sku, view, i+1, n, engine, attempt)
        ...
        if engine == "nano_pro" and clients.get("gemini"):
            image_bytes = generate_gemini(
                clients["gemini"], source_path, prompt,
                model=GEMINI_PRO, enhanced=(attempt > 1),
            )
            ...
            image_bytes = generate_gemini(
                clients["gemini"], source_path, variant_prompt,
                model=GEMINI_PRO, enhanced=(attempt > 1),
            )
            model_used = GEMINI_PRO
        ...
        if image_bytes and quality_gate(image_bytes, sku, view):
            cand = _save_candidate(image_bytes, sku, view, engine, model_used, attempt, feedback_applied)
