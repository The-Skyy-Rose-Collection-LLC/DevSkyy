...
def hybrid_composite_from_dna(
    ...
    for region in regions_to_add:
        ...
        bbox = detect_logo_region_via_vision(anthropic_client, source_image_path, region["description"])
        if bbox is None:
    ...
    log.info("Saved composite: %s", output_path.relative_to(PROJECT_ROOT) if output_path.is_relative_to(PROJECT_ROOT) else output_path)
    return output_path
...
def should_use_composite_fallback(tournament_result) -> bool:
    ...
    avg_text = sum(j.text_accuracy for j in tournament_result.judges) / len(tournament_result.judges)
    avg_logo = sum(j.logo_accuracy for j in tournament_result.judges) / len(tournament_result.judges)
