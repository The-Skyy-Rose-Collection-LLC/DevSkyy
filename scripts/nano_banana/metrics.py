...
def score_color_match(source_path: Path, candidate_path: Path, target_hex: str) -> MetricResult:
    ...
    try:
        ...
        return MetricResult("color_match", False, 0.0, 0.0, COLOR_DELTA_E_THRESHOLD, {"error": str(exc)})
...
def score_ssim(source_path: Path, candidate_path: Path) -> MetricResult:
    ...
    try:
        ...
        return MetricResult("structural_similarity", False, 0.0, 0.0, SSIM_THRESHOLD, {"error": str(exc)})
