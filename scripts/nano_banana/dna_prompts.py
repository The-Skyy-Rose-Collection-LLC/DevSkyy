...
def build_feedback_prompt(
    ...
    for metric, details in failed_metrics.items():
        if metric == "color_match":
            ...
            fixes.append(f"- COLOR FIX: base color was off by ΔE={delta:.1f}. Use EXACT hex from spec.")
            ...
            got = details.get("got", "")
            fixes.append(f"- GARMENT FIX: rendered as '{got}', must be '{expected}'. Fix garment type.")
