...
def calculate_module_coverage(data: dict) -> dict[str, dict]:
    ...
    module_stats = defaultdict(lambda: {"lines_covered": 0, "total_lines": 0, "files": 0, "coverage": 0.0})
    ...
    for file_path, file_data in files.items():
        ...
        if len(path_parts) > 1:
            module = (
                path_parts[0]
                if path_parts[0] not in ["home", "user", "DevSkyy"]
                else path_parts[-2] if len(path_parts) > 2 else "root"
            )
        else:
            module = "root"
...
def graph_1_module_coverage_bar(module_stats: dict):
    ...
    bars = plt.bar(
        range(len(modules)),
        coverages,
        color=["#2ecc71" if c >= TARGET_COVERAGE else "#e74c3c" if c < 50 else "#f39c12" for c in coverages],
    )
...
def graph_8_coverage_trend():
    ...
    # Add target line
    plt.axhline(y=TARGET_COVERAGE, color="red", linestyle="--", linewidth=2, label=f"Target ({TARGET_COVERAGE}%)")
