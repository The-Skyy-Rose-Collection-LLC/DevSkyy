...
def analyze_with_gemini(client, image_path: Path) -> dict:
    ...
    try:
        response = client.models.generate_content(
            ...
            contents=[
                types.Part.from_bytes(data=image_path.read_bytes(), mime_type=_load_image_b64(image_path)[1]),
                EXTRACTION_PROMPT,
            ],
...
def consensus_vote(results: dict[str, dict]) -> tuple[dict, list[dict]]:
    ...
    all_fields = set()
    valid_results = {k: v for k, v in results.items() if "_error" not in v and "_parse_error" not in v}
