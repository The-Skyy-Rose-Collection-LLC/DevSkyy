...
async def generate_3d_model(image_path: Path, api_key: str, output_dir: Path) -> dict:
    ...
    try:
        with urllib.request.urlopen(req, context=ctx, timeout=60) as resp:  # nosec B310 — URL from controlled API response, not user input
    ...
    for _i in range(max_polls):
        ...
        try:
            with urllib.request.urlopen(poll_req, context=ctx, timeout=30) as resp:  # nosec B310 — URL from controlled API response, not user input
    ...
    try:
        ...
        with urllib.request.urlopen(download_req, context=ctx, timeout=120) as resp:  # noqa: SIM117  # nosec B310 — URL from controlled API response, not user input
...
async def main():
    ...
    try:
        with urllib.request.urlopen(bal_req, context=ctx, timeout=10) as resp:  # nosec B310 — URL from controlled API response, not user input
