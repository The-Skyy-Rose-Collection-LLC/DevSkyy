...
class CodexOrchestrator:
    ...
    def self_healing(self):
        ...
        if self._self_healing is None:
            try:
                from agent.modules.backend.universal_self_healing_agent import UniversalSelfHealingAgent
    ...
    async def _generate_healing_strategy(
        self, code: str, issues: List[Dict], language: str, context: Optional[Dict] = None
    ) -> Dict[str, Any]:
...
Respond with a structured healing strategy."""
            ...
            strategy_response = await self.codex.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert code healing strategist."},
            ...
            return {"strategy": "Apply fixes sequentially", "priority_issues": issues, "issue_groups": [issues]}
