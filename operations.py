...
class OperationsAgent(SuperAgent):
    ...
    def _register_tools(self) -> None:
        ...
        @self.registry.tool(
            ...
            input_schema={
                "type": "object",
                "properties": {
                    "page_type": {"type": "string", "enum": ["home", "collection", "product", "about", "contact"]},
                    "collection": {"type": "string", "enum": ["BLACK_ROSE", "LOVE_HURTS", "SIGNATURE"]},
                    "title": {"type": "string"},
                    "include_sections": {
                        "type": "array",
                        "items": {"type": "string"},
                                ...
                                "elements": [
                                    {
                                        ...
                                        "settings": {
                                            ...
                                            "typography_font_family": brand["typography"]["heading"],
                                        },
                                    },
                                ],
                            },
        ...
        self._tools["generate_elementor_template"] = self.registry.get("generate_elementor_template")
        ...
        self._tools["validate_elementor_template"] = self.registry.get("validate_elementor_template")
        ...
        @self.registry.tool(
            ...
            input_schema={
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["import_template", "export_template", "sync_products", "sync_media"]},
                    "template_id": {"type": "string"},
    ...
    async def _retrieve(self, request: Dict[str, Any], plan: List[PlanStep], context: ToolCallContext) -> RetrievalContext:
        collection = request.get("collection", "SIGNATURE")
        brand = BRAND_KIT.get(collection, BRAND_KIT["SIGNATURE"])
    ...
    async def _execute_step(self, step: PlanStep, retrieval_context: RetrievalContext, context: ToolCallContext) -> ExecutionResult:
        import time
        start_time = time.perf_counter()
        try:
            result = await self.registry.execute(step.tool_name, step.inputs, context)
            return ExecutionResult(tool_name=step.tool_name, success=True, result=result, duration_seconds=time.perf_counter() - start_time)
        except Exception as e:
            return ExecutionResult(tool_name=step.tool_name, success=False, error=str(e), duration_seconds=time.perf_counter() - start_time)
    
    async def _validate(self, results: List[ExecutionResult], context: ToolCallContext) -> ValidationResult:
        errors = []
        warnings = []
    ...
    async def _emit(self, results: List[ExecutionResult], validation: ValidationResult, context: ToolCallContext) -> Dict[str, Any]:
        return {
            "status": "success",
