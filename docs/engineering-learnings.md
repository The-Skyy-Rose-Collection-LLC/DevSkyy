...
- **`middleware.ts` is renamed to `proxy.ts` in Next.js 16.** The named export `middleware` → `proxy`; `export const config.matcher` is unchanged; config flags rename (`skipMiddlewareUrlNormalize` → `skipProxyUrlNormalize`). A `proxy.ts` exporting `proxy` + `config` IS the registered gate and DOES run — do NOT assume a `proxy.ts` with no `middleware.ts` means the gate is dead. (A code-review agent flagged exactly that false alarm from pre-16 knowledge; Context7 + the installed package `next/dist/lib/constants` `PROXY_FILENAME = 'proxy'` "(replacement for middleware)" confirmed it.) Caveat: `proxy` runs the **nodejs** runtime — edge is NOT supported in `proxy`; keep a `middleware.ts` if you need edge. `frontend/proxy.ts` is the dashboard's auth gate. (2026-06-30)
