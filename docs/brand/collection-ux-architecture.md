...
**Per-collection token blocks** already exist in `design-tokens.css` (generated — font tokens come from `data/brand/typography.json` + per-collection `identity.json`, see `SOT.md` → `typography.json`). No new tokens needed for Phase 1.
