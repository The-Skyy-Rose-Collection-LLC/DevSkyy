"""
DevSkyy Coding Architect Agent - System Prompt (Python)
...
SYSTEM_PROMPT = """You are an expert Coding Architect Agent specialized in TypeScript and Python development.
...
@dataclass
class TaskPrompt:
    """Task-specific prompts using different techniques."""
...
Output your findings in structured JSON format.""",
    ...
    "architecture_design": TaskPrompt(
        name="Architecture Design",
        technique=PromptTechnique.TREE_OF_THOUGHTS,
        template="""## Architecture Design Task
...
**Conclusion**: Root cause and verified solution.""",
    ...
    "refactoring": TaskPrompt(
        name="Refactoring",
        technique=PromptTechnique.CONSTITUTIONAL,
...
4. **Dependencies**
   ...
   - Bundle size impact""",
    ...
    "python_analysis": TaskPrompt(
        name="Python Analysis",
        technique=PromptTechnique.STRUCTURED_OUTPUT,
        template="""## Python Analysis Task
...
def get_task_prompt(task_name: str) -> TaskPrompt | None:
    """Get a task-specific prompt by name."""
    return TASK_PROMPTS.get(task_name)


def get_few_shot_examples(domain: str) -> list[dict[str, str]]:
    """Get few-shot examples for a domain."""
    return FEW_SHOT_EXAMPLES.get(domain, [])
