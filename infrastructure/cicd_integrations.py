...
class CICDIntegrationManager:
    ...
    async def _handle_github_webhook(
        ...
        if workflow_run:
            ...
            status_mapping = {
                'completed': PipelineStatus.SUCCESS if workflow_run.get('conclusion') == 'success' else PipelineStatus.FAILED,
    ...
    def _is_fashion_related_pipeline(self, event: PipelineEvent) -> bool:
        ...
        fashion_keywords = [
            'fashion', 'trend', 'style', 'apparel', 'clothing',
            'retail', 'ecommerce', 'inventory', 'product-catalog'
        ]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_health = 'healthy' if all(
                status in ['healthy', 'disabled'] for status in connection_health.values()
            ) else 'degraded'
