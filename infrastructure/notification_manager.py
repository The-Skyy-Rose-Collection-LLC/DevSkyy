...
class NotificationManager:
    ...
    def _setup_default_templates(self):
        ...
        self.add_template(NotificationTemplate(
            ...
            fields=[
                ...
                {"name": "Correlation ID", "value": "{correlation_id}", "inline": True}
            ]
        ))
        ...
        self.add_template(NotificationTemplate(
            ...
            emoji="👗",
            fields=[
                {"name": "Target Demographic", "value": "{target_demographic}", "inline": True},
                {"name": "Price Range", "value": "{price_range}", "inline": True},
                {"name": "Sustainability Score", "value": "{sustainability_score}", "inline": True}
            ],
    ...
    async def _handle_delivery_error(self, notification: NotificationMessage, error: str):
        ...
        if notification.retry_count <= notification.max_retries:
            ...
            logger.warning(f"Notification {notification.id} failed, retrying in {retry_delay}s (attempt {notification.retry_count}/{notification.max_retries})")
