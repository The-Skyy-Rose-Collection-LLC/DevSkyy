...
- Generating shoot assets that will flow into `skyyrose/elite_studio` for the imagery pipeline
...
- Palette: Dark `#0A0A0A` dominant, Silver `#C0C0C0` accent details, deep charcoal and slate.
...
- Palette: Crimson `#DC143C` hits against dark grounds. Deep reds and near-blacks.
...
- Palette: Gold `#D4AF37` in light and accessories, warm neutrals, cream, amber.
...
- **Never type-render the collection name in the photograph.** The lockup image (`assets/images/hero-overlays/`) is composited at the Elite Studio stage — burning it in photo makes it uncorrectable.
...
- **Photographer unfamiliar with the brand:** Share `docs/brand/visual-references.md` and send them 5 reference images before the shoot. Do not rely on the brief alone.
