...
## SkyyRose Engine Canon

**`openai/gpt-image-2` (high fidelity) is the locked canonical engine for all SkyyRose product and still imagery** (`project_imagery_engine_oai2`) — chosen for repeatable, identity-preserving product renders. Every other model below is **non-product / experimental**: use only for concept exploration, social graphics, or non-catalog visuals — never for product imagery that ships to skyyrose.co.

### STOP-AND-SHOW cost gate (mandatory before any paid generation)

Image generation costs money. Before running ANY `belt app run` that generates, print this manifest and wait for an explicit `y`:

```
STOP — Confirm before proceeding:

Action : image generation (<model app id>)
SKU    : <sku or "non-product">
Source : <exact source image path, if editing>
Cost   : ~$<estimate>
Proceed? [y/N]
```

Do not dispatch the generation until the user answers `y` (project CLAUDE.md STOP-AND-SHOW protocol).
...
> **Product imagery → `openai/gpt-image-2` only.** All other models in this table are non-product / experimental (concept art, social graphics, non-catalog visuals). Topaz Upscaler may post-process any approved render.
