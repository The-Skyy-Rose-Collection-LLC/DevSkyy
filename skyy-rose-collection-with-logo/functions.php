...
/**
 * Display Skyy Rose Collection Logo
 */
if (!function_exists('skyy_rose_display_logo')) {
    function skyy_rose_display_logo($size = '60h') {
        $logo_path = get_template_directory_uri() . '/assets/images/TSRC-logo-' . $size . '.gif';
        $logo_file = get_template_directory() . '/assets/images/TSRC-logo-' . $size . '.gif';

        // Check if the logo file exists
        if (!file_exists($logo_file)) {
            // Fallback to 60h version
            $logo_path = get_template_directory_uri() . '/assets/images/TSRC-logo-60h.gif';
            $size = '60h';
        }

        // Extract dimensions from size
        $dimensions = array(
            '40h' => array('width' => 40, 'height' => 40),
            '50h' => array('width' => 50, 'height' => 50),
            '60h' => array('width' => 60, 'height' => 60),
            '80h' => array('width' => 80, 'height' => 80),
        );

        $width = isset($dimensions[$size]) ? $dimensions[$size]['width'] : 60;
        $height = isset($dimensions[$size]) ? $dimensions[$size]['height'] : 60;

        return array(
            'url' => $logo_path,
            'width' => $width,
            'height' => $height,
            'alt' => get_bloginfo('name') . ' - ' . esc_attr__('Logo', 'skyy-rose-collection')
        );
    }
}
...
if (!function_exists('skyy_rose_get_logo_html')) {
    function skyy_rose_get_logo_html() {
        $logo = skyy_rose_display_logo('60h');

        $html = '<div class="skyy-rose-logo-container">';
        $html .= '<a href="' . esc_url(home_url('/')) . '" rel="home" class="skyy-rose-logo-link" aria-label="' . esc_attr(get_bloginfo('name')) . ' - ' . esc_attr__('Go to homepage', 'skyy-rose-collection') . '">';

        // Responsive logo with different sizes for different screens
        $html .= '<picture class="skyy-rose-logo-picture">';
        $html .= '<source media="(max-width: 480px)" srcset="' . esc_url(str_replace('60h', '40h', $logo['url'])) . '">';
        $html .= '<source media="(max-width: 768px)" srcset="' . esc_url(str_replace('60h', '50h', $logo['url'])) . '">';
        $html .= '<img src="' . esc_url($logo['url']) . '" ';
        $html .= 'alt="' . esc_attr($logo['alt']) . '" ';
        $html .= 'class="skyy-rose-logo-gif" ';
        $html .= 'width="' . esc_attr($logo['width']) . '" ';
        $html .= 'height="' . esc_attr($logo['height']) . '" ';
        $html .= 'loading="eager">';
        $html .= '</picture>';

        $html .= '<span class="skyy-rose-logo-text">';
        if (is_front_page() && is_home()) {
            $html .= '<span class="site-title-h1">' . get_bloginfo('name') . '</span>';
        } else {
