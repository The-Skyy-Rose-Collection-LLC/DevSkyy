...
if ( class_exists( 'WooCommerce' ) ) {
	...
	foreach ( $wc_products as $idx => $wc_product ) {
		...
		$products_per_room[ $room_idx ][] = array(
			...
			'collection' => __( 'Love Hurts Collection', 'skyyrose' ),
			'sizes'      => '',
			'left'       => (string) $pos[0],
			'top'        => (string) $pos[1],
			'prop'       => 'cathedral-display',
			'prop_label' => __( 'Cathedral display', 'skyyrose' ),
		);
	}
} else {
	...
	$fallback = array(
		...
		array(
			'name'  => 'Glass Dome Varsity',
			'price' => '$295',
			'room'  => 1,
		),
		array(
			'name'  => 'Last Petal Joggers',
			'price' => '$135',
			'room'  => 1,
		),
	);
...
$rooms = array(
	array(
		'name'     => esc_html__( 'The Bedroom', 'skyyrose' ),
		'image'    => 'scene-love-hurts-bedroom.webp',
		'products' => $products_per_room[0],
	),
	array(
		'name'     => esc_html__( 'The Glass Dome', 'skyyrose' ),
