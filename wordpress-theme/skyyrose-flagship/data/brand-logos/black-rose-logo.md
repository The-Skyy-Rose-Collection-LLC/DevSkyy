---
logo_id: black-rose
collection: black-rose
image_path: data/brand-logos/black-rose-logo.jpeg
---

# Black Rose Logo — Canonical Reference

This is the SINGLE source of truth for the Black Rose logo art. Every dossier
in the `black-rose` collection that says "the Black Rose logo" references
this file. Update here and the change propagates to every product.

## Visual Description

A vertical cluster of three illustrated roses on a cloud, with thorny green
vines wrapping around the stems:

- **Three roses, vertically stacked, illustrated comic-book style** with hard
  ink edges and soft tonal shading on the petals.
  - One **large rose bloom** dominant in the upper portion of the composition,
    fully open, viewed face-on.
  - One **medium bloom** on the upper-left, three-quarter angle, slightly smaller.
  - One **smaller bud or partially-open rose** lower in the cluster.
  - A few **leaves** behind / between the blooms.
- **Petals are greyscale** — light grey at the highlights, dark grey to near-black
  at the shadows, with bold ink outlines. NOT photo-realistic. NOT solid black.
  NOT solid white.
- **Stems are bright kelly green** — saturated, slightly cartoony, with darker
  green shading on the underside of each stem.
- **Thorny vines** wrap around the stems and twist between the blooms. Thorns
  are clearly visible — short, sharp, dark-green spikes along the stems.
- **A small white-and-light-blue cumulus cloud** sits at the very base of the
  cluster, like a pedestal under the stems. The cloud has soft pillowy lobes
  and subtle pale-blue shading on the underside.
- The composition is **roughly oval/teardrop overall** — wider near the top
  (the rose blooms), narrower at the bottom (where stems meet the cloud),
  bottom anchored by the cloud.

## What this logo IS NOT

- NOT a single rose silhouette.
- NOT a solid-black tonal mark.
- NOT a "B" or "BR" monogram (those are different logos in the brand system).
- NOT the abstract photographic black-rose-with-gold-accents at
  `assets/branding/black-rose-logo.png` (that file is a hero treatment for
  collection landing pages, not the product brandable logo).

## How techniques apply this logo

Different products apply this logo via different techniques. The art stays
the same; the texture/colorway changes per technique:

- **Embossed** (e.g., Black Rose Crewneck): The art is applied as a raised
  dimensional relief on the fabric surface — visibly 3D and tactile, NOT
  debossed/sunken, NOT a flat print. The embossed colorway is a reduced
  3-color palette: **BLACK** (rose blooms), **WHITE** (cumulus cloud at base),
  and **GREY** (stems, leaves, thorny vines). NO kelly-green stems on the
  embossed version, NO blue cloud shading on the embossed version — those
  canonical colors only appear in techniques that preserve the full palette
  (silicone appliqué, screen-print, etc.). The full canonical art SHAPES are
  preserved (three roses + leaves + stems + thorny vines + cumulus cloud);
  only the colorway is reduced.
- **Silicone appliqué** (e.g., Black Rose Joggers): Raised silicone patch
  with full multi-color art preserved.
- **Embroidered**: Thread version of the art. Colors approximated in thread.
- **Embroidered patch**: Same as embroidered, stitched onto a fabric backing
  patch sewn onto the garment.
- **Printed / DTG / screen-print**: Flat-ink full-color version.
- **Tackle twill**: Felt/twill cut-letter or cut-shape appliqué (see
  Authentic Collection patch for usage).

## Reference Image

Image file: `data/brand-logos/black-rose-logo.jpeg`. Pipelines that build RAS
prompts attach this image as a third reference (after the 3D scaffold and the
techflat) when rendering any product whose dossier names this logo.
