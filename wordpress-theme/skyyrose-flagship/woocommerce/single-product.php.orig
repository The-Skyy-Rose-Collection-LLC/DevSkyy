...
while ( have_posts() ) :
	...
	<main id="product-<?php the_ID(); ?>" <?php wc_product_class( 'sr-product', $product ); ?>
		...
		<!-- BREADCRUMB -->
		<nav class="sr-breadcrumb" aria-label="<?php esc_attr_e( 'Breadcrumb', 'skyyrose' ); ?>">
			<div class="sr-container">
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>">SkyyRose</a>
				<span class="sr-bc-sep">/</span>
				<a href="<?php echo esc_url( $shop_url ); ?>"><?php esc_html_e( 'Shop', 'skyyrose' ); ?></a>
				<span class="sr-bc-sep">/</span>
			...
			<div class="sr-container sr-hero-grid">
				...
				<div class="sr-gallery" data-gallery>
					...
					<div class="sr-gallery-main">
						<?php if ( $meta['limited'] ) : ?>
							<span class="sr-badge-limited">
							<?php
								esc_html_e( 'Limited Edition', 'skyyrose' );
							if ( $meta['edition_of'] ) {
						...
						<div class="sr-gallery-thumbs">
							...
							foreach ( $gallery_ids as $gid ) :
								...
								<button type="button" class="sr-thumb" data-img="<?php echo esc_url( $gurl ); ?>"
										aria-label="<?php printf( esc_attr__( 'View image %d', 'skyyrose' ), (int) $sr_thumb_idx ); ?>">
					...
					<div class="sr-info-inner">
						...
						<?php if ( $meta['material'] || $meta['fit'] || $meta['detail'] ) : ?>
							<div class="sr-info-specs">
								...
								<?php if ( $meta['detail'] ) : ?>
									<div class="sr-spec">
										<span class="sr-spec-label"><?php esc_html_e( 'Detail', 'skyyrose' ); ?></span>
						...
						<div class="sr-stock sr-stock-<?php echo esc_attr( $stock_status ); ?>">
							<?php if ( 'instock' === $stock_status ) : ?>
								<span class="sr-stock-dot"></span> <?php esc_html_e( 'In Stock — Ready to Ship', 'skyyrose' ); ?>
							<?php elseif ( 'onbackorder' === $stock_status ) : ?>
								<span class="sr-stock-dot"></span> <?php esc_html_e( 'Pre-Order — Ships Spring 2026', 'skyyrose' ); ?>
							<?php else : ?>
								<span class="sr-stock-dot"></span> <?php esc_html_e( 'Sold Out', 'skyyrose' ); ?>
							<?php endif; ?>
						</div>
						...
						<!-- Social Share -->
						<div class="sr-share" aria-label="<?php esc_attr_e( 'Share this product', 'skyyrose' ); ?>">
							<span class="sr-share-label"><?php esc_html_e( 'Share', 'skyyrose' ); ?></span>
							<div class="sr-share-btns">
								<button class="sr-share-btn sr-share-native" type="button" aria-label="<?php esc_attr_e( 'Share', 'skyyrose' ); ?>" style="display:none"
								...
								<a class="sr-share-btn sr-share-x" href="https://twitter.com/intent/tweet?url=<?php echo rawurlencode( get_permalink() ); ?>&text=<?php echo rawurlencode( get_the_title() . ' — ' . $config['label'] . ' by @SkyyRose. Luxury Grows from Concrete.' ); ?>" target="_blank" rel="noopener noreferrer" aria-label="<?php esc_attr_e( 'Share on X', 'skyyrose' ); ?>">
									<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
								</a>
								<a class="sr-share-btn sr-share-fb" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo rawurlencode( get_permalink() ); ?>" target="_blank" rel="noopener noreferrer" aria-label="<?php esc_attr_e( 'Share on Facebook', 'skyyrose' ); ?>">
									<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
								</a>
								<button class="sr-share-btn sr-share-copy" type="button" aria-label="<?php esc_attr_e( 'Copy link', 'skyyrose' ); ?>" data-url="<?php echo esc_attr( get_permalink() ); ?>">
			...
			<div class="sr-container">
				<div class="sr-details-grid">
					...
					<?php if ( $meta['material'] || $meta['care'] || $meta['made_in'] ) : ?>
						<div class="sr-accordion" data-accordion>
							<button type="button" class="sr-accordion-trigger" aria-expanded="false">
								<span><?php esc_html_e( 'Materials & Care', 'skyyrose' ); ?></span>
								<span class="sr-accordion-icon">+</span>
							</button>
							<div class="sr-accordion-content">
								<?php if ( $meta['material'] ) : ?>
									<p><strong><?php esc_html_e( 'Material:', 'skyyrose' ); ?></strong> <?php echo esc_html( $meta['material'] ); ?></p>
								<?php endif; ?>
								<?php if ( $meta['made_in'] ) : ?>
									<p><strong><?php esc_html_e( 'Made in:', 'skyyrose' ); ?></strong> <?php echo esc_html( $meta['made_in'] ); ?></p>
								<?php endif; ?>
								<?php if ( $meta['care'] ) : ?>
									<p><strong><?php esc_html_e( 'Care:', 'skyyrose' ); ?></strong> <?php echo esc_html( $meta['care'] ); ?></p>
								<?php endif; ?>
							</div>
						</div>
					<?php endif; ?>
					...
					<div class="sr-accordion" data-accordion>
						...
						<div class="sr-accordion-content">
							<p><?php esc_html_e( 'All SkyyRose pieces are designed gender-neutral. We recommend ordering your usual size for a standard fit, or sizing up for an oversized look.', 'skyyrose' ); ?></p>
							<p>
							<?php
								printf(
									/* translators: %s: contact email */
									esc_html__( 'Need help? Email %s with your height and weight for a personal recommendation.', 'skyyrose' ),
					...
					<div class="sr-accordion" data-accordion>
						...
						<div class="sr-accordion-content">
							<p><?php esc_html_e( 'Free shipping on orders over $150. Standard delivery 5-7 business days. Expedited options available at checkout.', 'skyyrose' ); ?></p>
							<p><?php esc_html_e( '30-day return policy on unworn items with original tags. Pre-order items ship on the announced date.', 'skyyrose' ); ?></p>
						</div>
					</div>
				</div>
			</div>
		...
		<?php if ( ! empty( $related ) ) : ?>
			<section class="sr-related" aria-label="<?php esc_attr_e( 'Related Products', 'skyyrose' ); ?>">
				<div class="sr-container">
					<div class="sr-related-head">
						<h2 class="sr-related-title">
						<?php
							printf(
								/* translators: %s: collection name */
								esc_html__( 'More from %s', 'skyyrose' ),
						...
						<?php if ( $cat_link && ! is_wp_error( $cat_link ) ) : ?>
							<a href="<?php echo esc_url( $cat_link ); ?>" class="sr-related-link">
								<?php esc_html_e( 'View Full Collection', 'skyyrose' ); ?> &rarr;
							</a>
						<?php endif; ?>
					</div>
						...
						foreach ( $related as $rel_product ) :
							...
							<a href="<?php echo esc_url( $rel_link ); ?>" class="sr-related-card">
								<div class="sr-related-img">
									...
									<div class="sr-related-hov"><span><?php esc_html_e( 'View Piece', 'skyyrose' ); ?></span></div>
								</div>
			...
			<div class="sr-container sr-cta-inner">
				...
				<?php if ( $cat_link && ! is_wp_error( $cat_link ) ) : ?>
					<a href="<?php echo esc_url( $cat_link ); ?>" class="sr-cta-btn">
						<?php esc_html_e( 'Shop Full Collection', 'skyyrose' ); ?>
					</a>
				<?php endif; ?>
			</div>
		</section>
		...
		<section class="sr-recently-viewed" aria-label="<?php esc_attr_e( 'Recently Viewed', 'skyyrose' ); ?>" style="display:none">
			<div class="sr-container">
				<div class="sr-rv-head">
					<h2 class="sr-rv-title"><?php esc_html_e( 'Recently Viewed', 'skyyrose' ); ?></h2>
				</div>
		...
		<div class="sr-sticky-atc" id="srStickyATC" aria-hidden="true">
			<div class="sr-container sr-sticky-inner">
				...
				<a href="#sr-atc-anchor" class="sr-sticky-btn"><?php esc_html_e( 'Add to Bag', 'skyyrose' ); ?></a>
			</div>
		</div>
