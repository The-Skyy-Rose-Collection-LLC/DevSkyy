...
(function () {
	...
	function openModal(productId) {
		...
		if (imgEl) {
			imgEl.textContent = ''; // Clear safely.
			if (product.image) {
				...
				letterSpan.className = 'col-modal__letter';
				letterSpan.textContent = product.name ? product.name.charAt(0) : '';
				imgEl.appendChild(letterSpan);
			}
		}

		/* Sizes. */
		if (sizesEl) {
			sizesEl.textContent = ''; // Clear safely.
			var sizes = product.sizes || ['S', 'M', 'L', 'XL'];
			sizes.forEach(function (s) {
				var btn = document.createElement('button');
				btn.className = 'col-modal__sz';
