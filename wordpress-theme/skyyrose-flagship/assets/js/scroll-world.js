  ...
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        // A post-fetch decode failure must also latch s.failed — otherwise the
        // still stays hidden behind a dead <video> with no way to recover.
        v.addEventListener('error', () => { s.failed = true; s.ready = false; s.el.classList.remove('has-clip'); });
...
/* ============================================================================
   SkyyRose bootstrap — mount from a WP-localized global (SKYY_SCROLL_WORLD_CONFIG)
   or an inline global on a standalone page. Matches the theme's self-init-IIFE +
   localized-config convention (cf. skyy-3d.js reading SKYY_3D_CONFIG). No deps.
   ========================================================================== */
(function () {
  function boot() {
    var cfg = (typeof window !== 'undefined') && window.SKYY_SCROLL_WORLD_CONFIG;
    var mount = document.getElementById('skyyrose-collections-world');
    if (!cfg || !mount || typeof window.mountScrollWorld !== 'function') return;
    if (mount.getAttribute('data-sw-mounted')) return;
    mount.setAttribute('data-sw-mounted', '1');
    window.mountScrollWorld(mount, cfg);
  }
