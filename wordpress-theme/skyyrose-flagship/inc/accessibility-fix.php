...
class SkyyRose_Accessibility_Fix {
    ...
    public function __construct() {
        ...
        add_action('wp_enqueue_scripts', [$this, 'deduplicate_styles'], 999);
    }
    ...
    public function process($html) {
        ...
        $html = preg_replace_callback(
            '/<button\b((?:(?!type\s*=)[^>])*)>/is',
            function ($m) { return '<button type="button"' . $m[1] . '>'; },
            $html
        );
        ...
        $html = preg_replace_callback(
            ...
            function ($m) use (&$seen_ids) {
                $id = $m[2];
                if (!isset($seen_ids[$id])) { $seen_ids[$id] = 1; return $m[0]; }
                $seen_ids[$id]++;
        ...
        $html = preg_replace_callback(
            ...
            function ($m) {
                if (stripos($m[2] . $m[3], 'tabindex=') !== false) return $m[0];
        ...
        $html = preg_replace_callback(
            '/<a\b([^>]*?)>\s*<\/a>/is',
            function ($m) {
                if (stripos($m[1], 'aria-label') !== false) return $m[0];
                preg_match('/href="([^"]*)"/', $m[1], $h);
                $href = isset($h[1]) ? $h[1] : '';
                $label = 'Link';
                if (strpos($href, 'cart') !== false) $label = 'View cart';
                elseif (strpos($href, 'wishlist') !== false) $label = 'View wishlist';
                elseif (strpos($href, 'account') !== false) $label = 'My account';
                elseif (strpos($href, 'shop') !== false) $label = 'Shop';
        ...
        $html = preg_replace_callback(
            ...
            function ($m) {
                $combined = $m[1] . $m[2];
                if (stripos($combined, 'aria-label') !== false) return $m[0];
                if (preg_match('/\bid\s*=\s*"/', $combined)) return $m[0];
        ...
        $html = preg_replace_callback(
            '/<img\b([^>]*?)>/is',
            function ($m) {
                if (stripos($m[1], 'loading=') !== false) return $m[0];
                if (preg_match('/class="[^"]*(?:hero|logo|brand|monogram)/i', $m[1])) return $m[0];
