...
try:
    ...
    # Create a local FastAPI app for the API
    fastapi_app = FastAPI(
        title="DevSkyy API",
    ...
    @fastapi_app.get("/v1/agents", response_model=List[Dict[str, Any]])
    ...
    @fastapi_app.get("/v1/agents/{agent_id}")
    ...
    @fastapi_app.get("/v1/agents/{agent_id}/stats")
    ...
    @fastapi_app.get("/v1/agents/{agent_id}/tools")
    ...
    @fastapi_app.post("/v1/agents/{agent_id}/start")
    ...
    @fastapi_app.post("/v1/agents/{agent_id}/stop")
    ...
    @fastapi_app.get("/v1/metrics/dashboard", response_model=DashboardMetrics)
    ...
    print("Using minimal fallback handler...")

    def handler(event, context):
        """Minimal Lambda handler fallback."""
        path = event.get("path", "/")
        return {
            "statusCode": 200,
            "headers": {
            ...
            "body": json.dumps({
                "status": "ok",
                "message": "DevSkyy API (Minimal Mode)",
                "path": path,
                "note": "FastAPI/Mangum not available - install dependencies",
            }),
        }
