...
class WebhookManager:
    ...
    async def _deliver(
        ...
        try:
            ...
            if attempt < self.config.max_retries:
                ...
                asyncio.create_task(
                    self._retry_delivery(endpoint, event, attempt + 1, delay)
                )
            else:
                # Move to dead letter
    ...
    def _check_rate_limit(self, endpoint_id: str) -> bool:
        ...
        self._delivery_counts[endpoint_id] = [
            t for t in self._delivery_counts[endpoint_id]
            if t > window_start
        ]
    ...
    async def retry_dead_letter(self, delivery_id: str) -> Optional[WebhookDelivery]:
