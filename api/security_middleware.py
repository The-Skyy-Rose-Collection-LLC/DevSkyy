    ...
    def analyze_request(self, request: Request) -> tuple[bool, Optional[str], Optional[str]]:
        ...
        for blocked_agent in self.blocked_user_agents:
            if blocked_agent in user_agent:
                return False, "blocked_user_agent", f"Blocked user agent: {blocked_agent}"
        ...
        for pattern in self.suspicious_patterns:
            if re.search(pattern, url_path + query_params, re.IGNORECASE):
                return False, "suspicious_pattern", f"Suspicious pattern detected: {pattern}"
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _validation_error_response(self, request_id: str, error: ValidationError) -> JSONResponse:
        ...
        return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content=response_data.dict())
    ...
    async def _log_request(
        self, request: Request, response: Response, request_id: str, client_ip: str, processing_time: float
    ):
