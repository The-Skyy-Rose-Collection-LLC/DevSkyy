...
async def get_dashboard_data(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve dashboard data: {str(e)}"
        )
...
async def get_recent_activities(request: Request, limit: int = 10):
    """Get recent system activities."""
    if hasattr(request.app, "state"):
        await dashboard_service.initialize(request.app.state)
