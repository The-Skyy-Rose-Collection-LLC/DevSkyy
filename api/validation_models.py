...
class AgentExecutionRequest(BaseModel):
    ...
    task_description: constr(
        min_length=1, 
        max_length=5000
    ) = Field(..., description="Task description (max 5000 chars)")
...
class ContentGenerationRequest(BaseModel):
    ...
    prompt: constr(
        min_length=1,
        max_length=10000
    ) = Field(..., description="Content generation prompt")
    
    target_audience: Optional[constr(max_length=200)] = Field(
        None, description="Target audience description"
    )
    
    tone: Optional[constr(max_length=50)] = Field(
        None, description="Content tone"
    )
