...
| Hub pixels | `hub-manifest.json` (`assets/hub/manifest.json`) | `assets/hub/ingest_verdicts.py`; `scripts/sync_hub_to_theme.py` | `skyyrose.core.asset_hub.verify_integrity()` — wired into `sot-status` only (not yet its own CI gate) | **Tree is real, wiring landed on main** (`a11ca3208`, "tie verified asset-hub to the SOT chain") — this SOT.md previously (pre-registry) described the hub as tracked-but-unwired; that was already stale by the time this registry was written. Caveat verified this session: hub-referenced pixel files live under `assets/hub/collections/**`, which is **gitignored** (`.gitignore:217`), so `verify_integrity()` reports DRIFT on any checkout that lacks those physical files — a fresh clone or CI runner will always show DRIFT here until the check or the asset-storage strategy changes. Not a regression; a property of gitignored pixels. |
