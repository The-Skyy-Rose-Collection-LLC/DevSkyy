...
| _no symlink (×4, GENERATED)_ → `data/collections/<slug>/sot.json` | `wordpress-theme/skyyrose-flagship/data/collections/{black-rose,love-hurts,signature,kids-capsule}/sot.json` | Assembled per-collection view the WP theme reads (identity + catalog + imagery + logos + palette + fonts, resolved). | **GENERATED — DO NOT EDIT.** `data/build-collection-sot.py` (canon = identity.json); verified by `data/verify-collection-sot.py`; byte-compare drift-guarded by `scripts/validate_catalog_consistency.py` (`collection_sot_current`); read via `inc/collection-sot-reader.php`; feeds `sot-images.json`. A test/pipeline that regenerates it in-tree must not commit the rewrite. |
| `lookbook-sot.json` | `wordpress-theme/skyyrose-flagship/data/lookbook-sot.json` | The one aggregated SOT for the `wordpress-theme/skyyrose-flagship` **lookbook** component. | **GENERATED — DO NOT EDIT.** `scripts/sot/lookbook.py` is the sole component owner; it reads only the ordered sources in `scripts/lookbook-manifest.json` and generates this artifact plus `docs/campaigns/sot-lookbook.html`. Root `build-lookbook-*.py` commands are compatibility entrypoints only. The watchdog and CI byte-compare both outputs; a missing canonical component is a failure. |
...
- `assets/brand/brand.yaml` — structured brand identity: tagline (active + retired history), legal_name / founder / origin / mission, semantic + neutral colors, social handles, prod/dashboard URLs. Generates `inc/brand.generated.php` via `scripts/sync_brand_to_php.py`. **Fonts are NOT owned here** — typography resolves via `typography.json` (registered above in Data SOT; its in-file `typography:` block is stale/dead and slated for removal). Collection metadata → `identity.json`, not here.
