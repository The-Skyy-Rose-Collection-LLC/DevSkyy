...
from api_integration.workflow_engine import workflow_engine, Workflow, WorkflowTrigger, WorkflowStep
from api_integration.workflow_engine import TriggerType, ActionType
...
class FashionAPIExamples:
    ...
    async def example_1_api_discovery_and_setup(self):
        ...
        print(f"✅ Discovered {sum(len(apis) for apis in discovered_apis.values())} APIs")
    ...
    async def example_2_fashion_trend_analysis(self):
        ...
        print(f"  Fashion Relevance Score: {context_analysis['fashion_relevance_score']:.2f}")
        print(f"  Identified Categories: {context_analysis['identified_categories']}")
        print(f"  Sustainability Mentions: {context_analysis['sustainability_mentions']}")
        ...
        print(f"✅ Synced {sync_result['trends_synced']} trends from {len(sync_result['sources'])} sources")
    ...
    async def example_4_api_gateway_usage(self):
        ...
        print(f"  Success Rate: {metrics['gateway_metrics']['successful_requests'] / max(metrics['gateway_metrics']['total_requests'], 1) * 100:.1f}%")
        print(f"  Average Response Time: {metrics['gateway_metrics']['average_response_time']:.3f}s")
    ...
    async def example_5_fashion_personalization(self):
        ...
        print(f"  Fashion Relevance: {customer_analysis['fashion_relevance_score']:.2f}")
        print(f"  Sustainability Focus: {len(customer_analysis['sustainability_mentions'])} mentions")
        print(f"  Style Categories: {[cat['category'] for cat in customer_analysis['identified_categories'][:3]]}")
        ...
        for rec in personalized_recs[:2]:  # Show top 2
            print(f"  🌟 {rec['trend_name']}")
            print(f"     Personalization Score: {rec.get('personalization_score', 0):.2f}")
    ...
    async def example_6_comprehensive_health_check(self):
        ...
        print(f"  Average Response Time: {gateway_health.get('average_response_time', 0):.3f}s")
        ...
        print(f"  Registered Workflows: {workflow_health.get('registered_workflows', 0)}")
        ...
        print(f"  Fashion APIs Health: {fashion_health.get('fashion_apis_healthy', 'N/A')}")
        ...
        print(f"  Trends Count: {intelligence_health['knowledge_base_stats']['trends_count']}")
        print(f"  Market Intelligence: {intelligence_health['knowledge_base_stats']['market_intelligence_count']}")
        
        # Overall system health
        print("\n📊 OVERALL SYSTEM HEALTH:")
        print("=" * 30)
        
        healthy_components = sum(1 for health in health_results.values() 
