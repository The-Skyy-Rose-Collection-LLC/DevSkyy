...
pwd_context = CryptContext(
    schemes=["bcrypt"], deprecated="auto", bcrypt__rounds=12  # NIST recommendation: 10-12 rounds
)
...
def create_refresh_token(user_id: str) -> str:
    ...
    payload = {"sub": user_id, "iat": now, "exp": exp, "type": "refresh", "jti": secrets.token_urlsafe(16)}
...
def verify_access_token(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if payload.get("type") != "access":
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token type (expected 'access')"
            )

        user_id: str = payload.get("sub")
        if user_id is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token missing user ID (sub claim)")
...
def verify_refresh_token(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if payload.get("type") != "refresh":
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token type (expected 'refresh')"
            )
...
def require_role(required_role: UserRole):
    ...
    async def role_checker(current_user: Dict = Depends(get_current_user)) -> Dict[str, Any]:
        ...
        if not user_roles:
            ...
            user_role_levels = [
                ROLE_HIERARCHY.get(UserRole(role), 0) for role in user_roles if role in [r.value for r in UserRole]
            ]
            ...
            if not user_role_levels:
                ...
                user_role = next(role for role, level in ROLE_HIERARCHY.items() if level == max_level)
...
async def require_scope(required_scope: str):
    ...
    async def scope_checker(current_user: Dict = Depends(get_current_user)):
        if required_scope not in current_user.get("scopes", []):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Scope '{required_scope}' required")
        return current_user
...
async def login(credentials: UserCredentials) -> AccessToken:
    ...
    access_token = create_access_token(user_id=user["id"], roles=user.get("roles", []))
