...
class AuditTrail:
    ...
    async def query(
        ...
        *,
        ...
        )  # nosec B608 — clauses contain only static literal strings; user values go in parameterized `params` list
        sql = (
    ...
    async def count(
        ...
        *,
        ...
        where = (
            f" WHERE {' AND '.join(clauses)}" if clauses else ""
        )  # nosec B608 — static literal clauses; user values in parameterized params list
