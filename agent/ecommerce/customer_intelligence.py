...
try:  # Optional dependency for computer vision analytics
    import cv2  # type: ignore[import-not-found]
except ImportError:  # pragma: no cover - fallback when OpenCV missing
    cv2 = None  # noqa: N816  # provide sentinel for downstream checks
...
class CustomerIntelligence:
    ...
    async def segment_customers(
        ...
        if customer_data:
            feature_keys = [
                key for key in customer_data[0].keys() if isinstance(customer_data[0][key], (int, float))
            ]
            feature_matrix = np.array(
                [[float(customer.get(key, 0.0)) for key in feature_keys] for customer in customer_data]
            )

            scaler = StandardScaler()
            scaled_data = scaler.fit_transform(feature_matrix)

            clustering = KMeans(n_clusters=n_segments, n_init="auto", random_state=42)
            labels = clustering.fit_predict(scaled_data)

            segments["total_customers"] = len(customer_data)
            for label in range(n_segments):
                mask = labels == label
                cluster_customers = feature_matrix[mask]
                if cluster_customers.size == 0:
                    continue

                centroid = cluster_customers.mean(axis=0)
                size = int(mask.sum())
                percentage = round(size / len(customer_data) * 100, 2)

                segments["segments"].append(
                    {
                        "id": f"Segment_{label + 1}",
                        "size": size,
                        "percentage": percentage,
                        "characteristics": {
                            feature_keys[i]: float(centroid[i]) for i in range(len(feature_keys))
                        },
                        "recommended_actions": [
                            "Deploy personalized marketing journeys",
                            "Offer tailored incentives based on behavior",
                            "Monitor engagement for churn signals",
                        ],
                    }
                )

            if not segments["segments"]:
                segments = self._generate_simulated_segments(n_segments)
        else:
            segments = self._generate_simulated_segments(n_segments)

        return segments

    def _generate_simulated_segments(self, n_segments: int) -> Dict[str, Any]:
        """Return simulated segment data when ML input unavailable."""
