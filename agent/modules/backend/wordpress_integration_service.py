...
class WordPressIntegrationService:
    ...
    async def get_site_theme_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = httpx.get(f"{self.api_base}/sites/{self.site_id}/themes/mine", headers=headers)
            response.raise_for_status()

            theme_data = response.json()

            # Also get site customization options
            customizer_response = httpx.get(f"{self.api_base}/sites/{self.site_id}/customizer", headers=headers)
