            ...
            if "client" in config or "api_key" in config:
                ...
                for prompt in test_prompts:
                    try:
                        ...
                        if platform == "claude":
                            response = await self._call_claude("sonnet", prompt, 500, 0.7)
                        elif platform == "openai":
                            response = await self._call_openai("gpt4", prompt, 500, 0.7)
                        elif platform == "gemini":
                            response = await self._call_gemini("pro", prompt, 500, 0.7)
