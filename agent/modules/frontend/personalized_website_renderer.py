...
async def main():
    ...
    for context in user_contexts:
        ...
        if result["status"] == "success":
            ...
            print(f"📊 Personalization Score: {result['personalization_score']:.0%}")
            print(f"\n📝 Content:")
            print(f"  Headline: {content['hero']['headline']}")
            print(f"  CTA: {content['hero']['cta_text']}")
            print(f"  Currency: {content['pricing']['currency']}")
            print(f"  Collections: {', '.join(content['collections'][:2])}")

            if content.get("offer"):
                print(f"\n🎁 Special Offer: {content['offer']['message']}")

            if content["messaging"].get("urgency"):
                print(f"⏰ Urgency: {content['messaging']['urgency']}")
