...
class ReActCapableMixin:
    ...
    def register_react_tool(self, func: Callable, name: str | None = None, description: str | None = None) -> None:
        """
    ...
    async def reason_and_act(
        ...
        try:
            ...
            trace.success = False
            result = {"final_answer": None, "error": str(e), "status": "failed"}
    ...
    async def _dspy_react_loop(
        ...
        react_module = dspy.ReAct(signature="task -> final_answer", tools=dspy_tools, max_iters=max_iterations)
        ...
        trace.steps.append(
            ReasoningStep(step_type=ReasoningStepType.FINAL_ANSWER, content=str(result.final_answer), confidence=0.9)
        )

        return {"final_answer": result.final_answer, "status": "success", "method": "dspy_react"}
    ...
    async def _fallback_react_loop(
        ...
        for iteration in range(max_iterations):
            ...
            thought = await self._generate_thought(
                task=task, observations=observations, context_str=context_str, iteration=iteration
            )
            ...
            if action and action.get("tool_name"):
                ...
                observations.append({"tool": tool_name, "args": tool_args, "result": observation})
    ...
    async def _generate_thought(
        self, task: str, observations: list[dict], context_str: str, iteration: int
    ) -> dict[str, Any]:
        """
        ...
        # Build tool list
        tool_list = "\n".join(
            [f"- {name}: {func.__doc__ or 'No description'}" for name, func in self._react_tools.items()]
        )
...
{{
    ...
    def configure_dspy(
        self, lm: Any = None, api_key: str | None = None, model: str = "claude-sonnet-4-5-20250929"
    ) -> bool:
        """
...
class IterativeRetrievalMixin:
        ...
        for iteration in range(max_iterations):
            ...
            if iteration < max_iterations - 1:
                current_query = await self._reformulate_query(
                    original_query=query, current_results=all_results, iteration=iteration
                )
    ...
    async def _reformulate_query(self, original_query: str, current_results: list[dict], iteration: int) -> str:
        """
