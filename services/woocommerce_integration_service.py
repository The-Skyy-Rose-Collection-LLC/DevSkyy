...
from services.seo_optimizer import SEOOptimizerService, ProductInfo, SEOMetaTags, SEOOptimizerError
...
class SEOCompliance(BaseModel):
    ...
    def is_compliant(self) -> bool:
        ...
        return (
            self.title_length_valid and
            self.description_length_valid and
            self.schema_markup_present and
            self.schema_markup_valid and
            self.focus_keywords_present
        )
...
class WooCommerceIntegrationService:
    ...
    def __init__(
        ...
        logger.info(
            "WooCommerceIntegrationService initialized",
            extra={"store_url": self.store_url}
        )
    ...
    def _generate_yoast_metadata(
        ...
        meta_data = [
            {
                "key": self.YOAST_TITLE_FIELD,
                "value": seo_tags.metatitle
            },
            {
                "key": self.YOAST_DESC_FIELD,
                "value": seo_tags.metadescription
            },
            {
                "key": self.YOAST_FOCUS_KW_FIELD,
                "value": keywords or ""
            },
            {
                "key": self.YOAST_LINKDEX_FIELD,
                "value": str(seo_score)
            },
            {
                "key": self.SCHEMA_MARKUP_FIELD,
                "value": json.dumps(schema_markup)
            }
        ]
    ...
    def _generate_fallback_metadata(self, product_data: ProductData) -> Dict:
        ...
        if len(description) < self.DESC_MIN_LENGTH:
            description = f"{product_data.name}. Premium {product_data.category}. Shop now for the best quality and prices."[:self.DESC_MAX_LENGTH]
    ...
    async def create_product_with_seo(self, product_data: ProductData) -> ProductWithSEOResponse:
        ...
        try:
            logger.info(
                ...
                extra={"product_name": product_data.name}
            )
            ...
            try:
                ...
                seo_tags = await self.seo_service.generate_seo_tags(
                ...
                logger.warning(
                    "SEO service failed, using fallback metadata",
                    extra={"error": str(e)}
                )
                fallback_used = True
            ...
            return ProductWithSEOResponse(
                success=False,
                error=f"WooCommerce API error: {e.response.status_code} - {e.response.text}"
            )
    ...
    async def update_product_seo(
        ...
        try:
            ...
            return ProductWithSEOResponse(
                success=False,
                error=f"SEO update failed: {str(e)}"
            )
