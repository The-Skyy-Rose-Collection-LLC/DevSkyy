...
- **[2026-05-03] ChromaDB `where` clauses must have exactly one top-level operator.** A flat dict `{"a": 1, "b": 2}` is rejected with `ValueError: Expected where to have exactly one operator`. Multi-key filters must wrap with `$and`: `{"$and": [{"a": 1}, {"b": 2}]}`. This bit `ChromaVectorStore.search()` when both `filter_metadata` and `namespace` were passed (fixed in `vector_store.py:438` — see bug-097, `tests/orchestration/test_vector_store_namespace.py::test_namespace_filter_composes_with_explicit_filter`). Single-key paths (only filter, only namespace, neither) are still flat — only multi-key needs `$and`.

- **[2026-05-03] Vector search cache (`VectorSearchCache`, 5-min TTL) does NOT invalidate on writes.** After `delete_documents()` or `add_documents()`, repeat searches with the same embedding+filter return cached stale results until the entry expires. Surfaced by `test_forget_removes_memory` in `tests/orchestration/test_agent_memory.py`. Tests around delete-then-recall must `await get_vector_search_cache().clear()` between operations. Production fix is non-trivial (selective invalidation by namespace, or eager flush on writes) — tracked but not yet wired.

- **[2026-05-03] Voyage AI integration is live.** `VoyageEmbeddingEngine` in `orchestration/embedding_engine.py` provides asymmetric document/query embeddings via the `voyageai>=0.2.4` SDK. The SDK is sync-only (no `AsyncClient`) — engine wraps `client.embed()` with `asyncio.to_thread`. `embed_text()`/`embed_batch()` use `input_type="document"`; `embed_query()` uses `input_type="query"`. Mismatching the type degrades retrieval quality 5–10% on MTEB. Test fixture pattern: monkeypatch `voyageai.Client` with a `MagicMock` whose `.embed.side_effect` returns `SimpleNamespace(embeddings=[...], total_tokens=N)`. See `tests/orchestration/test_voyage_embedding.py`.

- **[2026-05-03] Voyage free tier is 10K TPM / 3 RPM until a payment method is added.** Per `voyageai.error.RateLimitError`: keys without a payment method on the Voyage dashboard are throttled to 10K tokens/minute and 3 requests/minute, EVEN WHEN inside the 200M free-token budget. The full SkyyRose catalog (~32K–44K tokens) blows the TPM cap on a single batch. Adding a payment method to the org at https://dashboard.voyageai.com/ unlocks standard limits without losing free tokens. This bit the first end-to-end smoke test of `index_skyyrose_catalog.py` against real Voyage. See bug-098. **How to apply:** before running any whole-corpus Voyage indexing job (catalog, dossiers, docs corpus), confirm payment method is configured OR throttle to <10K tokens/min with explicit `time.sleep(60)` gates between batches.
