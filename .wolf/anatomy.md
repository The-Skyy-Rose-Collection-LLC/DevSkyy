# anatomy.md

> Auto-maintained by OpenWolf. Last scanned: 2026-04-19T17:39:18.588Z
> Files: 701 tracked | Anatomy hits: 0 | Misses: 0

## ../.claude/

- `settings.json` (~1500 tok)

## ../.claude/agents/

- `gsd-codebase-mapper.md` — hooks: (~4622 tok)
- `gsd-integration-checker.md` — Required Context (provided by milestone auditor) (~3794 tok)
- `gsd-nyquist-auditor.md` — GAPS FILLED (~1815 tok)
- `gsd-research-synthesizer.md` — hooks: (~2144 tok)

## ../.claude/plans/

- `eager-doodling-fog.md` — Skyy — 100% Identical 3D Character Pipeline (~1726 tok)
- `i-want-you-to-shiny-clarke.md` — Plan — Post-Hotfix Cleanup + Agent Config Fix (~5025 tok)
- `well-lets-audit-separately-humming-beacon.md` — Full Audit — Product Imagery, Web Copy, Branding (separate tracks) (~3362 tok)
- `well-we-need-the-mutable-starfish.md` — Plan: Permanently Fix `renders/preflight.py` Dual-Module Error + Harden Against Recurrence (~2007 tok)
- `whimsical-toasting-cook.md` — Plan: RAGAnything Commercial Integration (~1685 tok)
- `whoa-i-want-you-sleepy-rocket.md` — Strategic Fix: Deploy Script Commercial Makeover + Theme Bloat Cleanup (~3262 tok)

## ./

- `.coverage` (~32768 tok)
- `.dockerignore` — Docker ignore rules (~122 tok)
- `.eslintrc.cjs` — ESLint configuration (~954 tok)
- `.gitattributes` — Git attributes (~70 tok)
- `.gitignore` — Git ignore rules (~1533 tok)
- `.markdownlint.json` (~57 tok)
- `.mcp.json` (~36 tok)
- `.npmrc` (~6 tok)
- `.nvmrc` (~1 tok)
- `.plugin-fix-complete` (~113 tok)
- `.pre-commit-config.yaml` — Pre-commit Configuration - Optimized for Speed and Safety (~1529 tok)
- `.prettierrc.js` (~381 tok)
- `.vercelignore` — DevSkyy .vercelignore - Production Deployment (~444 tok)
- `alembic.ini` — Alembic configuration for DevSkyy (~163 tok)
- `autotrain_config.yaml` (~131 tok)
- `CLAUDE.md` — OpenWolf (~5957 tok)
- `conftest.py` — /*.py", (~500 tok)
- `deployment_summary.json` (~406 tok)
- `devskyy_mcp.py` (~1153 tok)
- `docker-compose.staging.yml` — Docker Compose: 14 services (~3431 tok)
- `docker-compose.yml` — Docker Compose services (~1914 tok)
- `docker-entrypoint.sh` — Enterprise startup script with error handling and logging (~1012 tok)
- `Dockerfile` — Docker container definition (~1158 tok)
- `Dockerfile.worker` — DevSkyy Worker - Lightweight build for background task processing (~813 tok)
- `fastmcp.config.json` (~287 tok)
- `fly.toml` — fly.toml - DevSkyy Enterprise Platform (~1070 tok)
- `init.sql` — DevSkyy Database Initialization (~1088 tok)
- `lint-staged.config.mjs` — Declares relPaths (~346 tok)
- `main_enterprise.py` — API: 4 endpoints (~3932 tok)
- `Makefile` — Make build targets (~1530 tok)
- `mypy.ini` — Declares of (~459 tok)
- `nginx-app.devskyy.app.conf` — Nginx configuration for app.devskyy.app (self-hosted alternative) (~1497 tok)
- `nginx.conf` — Nginx configuration (~1630 tok)
- `package.json` — Node.js package manifest (~2968 tok)
- `pnpm-lock.yaml` — pnpm lock file (~195082 tok)
- `pre-build-check.sh` — DevSkyy Pre-Build Verification (~2307 tok)
- `prometheus.yml` — DevSkyy - Prometheus Configuration (~790 tok)
- `pyproject.toml` — AI-driven multi-agent orchestration platform for enterprise e-commerce automation with WordPress/Woo (~3651 tok)
- `README.md` — Project documentation (~1429 tok)
- `requirements-dev.txt` — Development Dependencies (~85 tok)
- `requirements-full.txt` — DevSkyy - Full ML/3D Dependencies (~46 tok)
- `requirements-imagery.txt` — Nano Banana 2 — SkyyRose AI Image Pipeline (~130 tok)
- `requirements.txt` — Python dependencies (~109 tok)
- `run_generation.sh` — Set environment variables for Tripo3D generation (~104 tok)
- `skyyrose_clothing_barcodes.txt` (~459 tok)
- `vercel.json` — /*.ts": { (~525 tok)
- `wordpress-health-check-results.json` (~2024 tok)

## .agents/skills/aidesigner-frontend/

- `SKILL.md` (~1482 tok)

## .aidesigner/

- `.gitkeep` (~0 tok)
- `latest.json` (~266 tok)
- `mcp-latest.html` — SkyyRose | Experience Engine (~1739 tok)

## .aidesigner/runs/2026-04-07T00-48-35-448Z-experience-engine-dashboard/

- `design.html` — SkyyRose | Experience Engine (~1739 tok)
- `preview-error.txt` (~25 tok)
- `repo-context.json` (~280 tok)
- `request.json` (~30 tok)
- `summary.json` (~266 tok)

## .claude/

- `IMMEDIATE_REQUIREMENTS.md` — Immediate Requirements - READ THIS FIRST (~499 tok)
- `mcp-config.json` (~116 tok)
- `ralph-loop-iteration-2.md` — Ralph Loop Iteration 2 - MCP Server Loading Fix (~601 tok)
- `settings.json` (~498 tok)
- `settings.local.json` (~450 tok)

## .claude/agents/

- `aidesigner-frontend.md` (~1481 tok)
- `architect.md` — Your Role (~1573 tok)
- `build-error-resolver.md` — Build Error Resolver (~936 tok)
- `code-reviewer.md` — Review Process (~2187 tok)
- `database-reviewer.md` — Database Reviewer (~1079 tok)
- `deploy-and-verify.md` — Deploy & Verify Agent (~325 tok)
- `doc-updater.md` — Documentation & Codemap Specialist (~831 tok)
- `e2e-runner.md` — E2E Test Runner (~1022 tok)
- `loop-operator.md` — Mission (~231 tok)
- `planner.md` — Your Role (~1763 tok)
- `python-reviewer.md` — Review Priorities (~843 tok)
- `refactor-cleaner.md` — Refactor & Dead Code Cleaner (~677 tok)
- `security-reviewer.md` — Security Reviewer (~1110 tok)
- `tdd-guide.md` — Your Role (~721 tok)
- `wp-code-simplifier.md` — WordPress Code Simplifier (~265 tok)

## .claude/commands/

- `aidesigner.md` (~799 tok)
- `build-fix.md` — Build and Fix (~143 tok)
- `checkpoint.md` — Checkpoint Command (~145 tok)
- `code-review.md` — Code Review (~247 tok)
- `deploy-wp.md` (~68 tok)
- `e2e.md` — E2E Command (~240 tok)
- `eval.md` — Eval Command (~167 tok)
- `fix-and-learn.md` (~101 tok)
- `learn.md` — /learn - Extract Reusable Patterns (~164 tok)
- `orchestrate.md` — Orchestrate Command (~227 tok)
- `plan-exit-review.md` — Plan Review Mode (~2659 tok)
- `plan.md` — Plan Command (~234 tok)
- `refactor-clean.md` — Refactor Clean (~180 tok)
- `tdd.md` — TDD Command (~266 tok)
- `test-coverage.md` — Test Coverage (~166 tok)
- `update-codemaps.md` — Update Codemaps (~176 tok)
- `update-docs.md` — Update Documentation (~183 tok)
- `verify.md` — Verification Command (~161 tok)
- `wp-simplify.md` (~46 tok)

## .claude/contexts/

- `dev.md` — Development Context (~105 tok)
- `research.md` — Research Context (~154 tok)
- `review.md` — Code Review Context (~132 tok)

## .claude/hooks/

- `hooks.json` — Declares check (~3245 tok)

## .claude/hooks/claude-md-tracker/

- `session-summary.sh` — Stop Hook — Summarize CLAUDE.md staleness at session end (~539 tok)
- `test-hooks.sh` — Test harness for CLAUDE.md staleness tracking hooks (~3208 tok)
- `track-changes.sh` — PostToolUse Hook — Track structural changes for CLAUDE.md staleness detection (~1534 tok)

## .claude/hooks/memory-persistence/

- `pre-compact.sh` — PreCompact Hook - Save state before context compaction (~312 tok)
- `session-end.sh` — Stop Hook (Session End) - Persist learnings when session ends (~409 tok)
- `session-start.sh` — SessionStart Hook - Load previous context on new session (~492 tok)

## .claude/hooks/strategic-compact/

- `suggest-compact.sh` — Strategic Compact Suggester (~467 tok)

## .claude/rules/

- `agents.md` — Agent Orchestration (~406 tok)
- `coding-style.md` — Coding Style (~350 tok)
- `development-workflow.md` — Development Workflow (~510 tok)
- `git-workflow.md` — Git Workflow (~156 tok)
- `hooks.md` — Hooks System (~192 tok)
- `openwolf.md` (~313 tok)
- `patterns.md` — Common Patterns (~256 tok)
- `performance.md` — Performance Optimization (~400 tok)
- `prompt-techniques.md` — Prompt Technique Enhancement (Always Active) (~611 tok)
- `security.md` — Security Guidelines (~216 tok)
- `sequential-thinking.md` — Sequential Thinking (Auto-Trigger) (~329 tok)
- `testing.md` — Testing Requirements (~267 tok)

## .claude/skills/

- `code-review.md` — DevSkyy Code Review (~257 tok)
- `deploy.md` — DevSkyy Deployment Pipeline (~267 tok)
- `fix-linting.md` — DevSkyy Linting Auto-Fix (~232 tok)
- `mcp-health.md` — MCP Health Check (~263 tok)
- `rag-ingest.md` — RAG Batch Ingestion (~219 tok)
- `react-best-practices.md` — React Best Practices (~293 tok)
- `test-and-commit.md` — TDD Workflow Automation (~273 tok)

## .claude/skills/aidesigner-frontend/

- `SKILL.md` (~1482 tok)

## .claude/skills/api-design/

- `SKILL.md` — API Design Patterns (~3271 tok)

## .claude/skills/api-design/agents/

- `openai.yaml` (~71 tok)

## .claude/skills/article-writing/

- `SKILL.md` — Article Writing (~786 tok)

## .claude/skills/article-writing/agents/

- `openai.yaml` (~83 tok)

## .claude/skills/backend-patterns/

- `SKILL.md` — Backend Development Patterns (~3454 tok)

## .claude/skills/backend-patterns/agents/

- `openai.yaml` (~74 tok)

## .claude/skills/bundle/branding-design/brand-architecture/

- `SKILL.md` — Brand Architecture (~1559 tok)

## .claude/skills/bundle/branding-design/brand-audit/

- `SKILL.md` — Brand Audit (~1488 tok)

## .claude/skills/bundle/branding-design/brand-identity-guide/

- `SKILL.md` — Brand Identity Guide (~1384 tok)

## .claude/skills/bundle/branding-design/brand-positioning-statement/

- `SKILL.md` — Brand Positioning Statement (~1508 tok)

## .claude/skills/bundle/branding-design/brand-refresh/

- `SKILL.md` — Brand Refresh (~4331 tok)

## .claude/skills/bundle/branding-design/brand-tagline/

- `SKILL.md` — Brand Tagline (~1592 tok)

## .claude/skills/bundle/branding-design/brand-voice-guide/

- `SKILL.md` — Brand Voice Guide (~6509 tok)

## .claude/skills/bundle/branding-design/color-palette-generator/

- `SKILL.md` — Color Palette Generator (~1401 tok)

## .claude/skills/bundle/branding-design/executive-resume/

- `SKILL.md` — Executive Resume (~2316 tok)

## .claude/skills/bundle/branding-design/expert-positioning/

- `SKILL.md` — Expert Positioning (~1775 tok)

## .claude/skills/bundle/branding-design/icon-set-brief/

- `SKILL.md` — Icon Set Brief (~1407 tok)

## .claude/skills/bundle/branding-design/merch-design-brief/

- `SKILL.md` — Merch Design Brief (~1501 tok)

## .claude/skills/bundle/branding-design/mission-statement/

- `SKILL.md` — Mission Statement (~1848 tok)

## .claude/skills/bundle/branding-design/naming-workshop/

- `SKILL.md` — Naming Workshop (~1548 tok)

## .claude/skills/bundle/branding-design/packaging-brief/

- `SKILL.md` — Packaging Brief (~1553 tok)

## .claude/skills/bundle/branding-design/portfolio-page/

- `SKILL.md` — Portfolio Page (~1596 tok)

## .claude/skills/bundle/branding-design/presentation-template-guide/

- `SKILL.md` — Presentation Template Guide (~1462 tok)

## .claude/skills/bundle/branding-design/rebrand-plan/

- `SKILL.md` — Rebrand Plan (~1535 tok)

## .claude/skills/bundle/branding-design/signage-brief/

- `SKILL.md` — Signage Brief (~1432 tok)

## .claude/skills/bundle/branding-design/signature-talk/

- `SKILL.md` — Signature Talk (~1794 tok)

## .claude/skills/bundle/branding-design/speaking-one-sheet/

- `SKILL.md` — Speaking One-Sheet (~2079 tok)

## .claude/skills/bundle/branding-design/style-tile/

- `SKILL.md` — Style Tile (~1568 tok)

## .claude/skills/bundle/branding-design/unboxing-experience/

- `SKILL.md` — Unboxing Experience (~1555 tok)

## .claude/skills/bundle/branding-design/visual-identity-brief/

- `SKILL.md` — Visual Identity Brief (~1542 tok)

## .claude/skills/bundle/content-copywriting/about-page/

- `SKILL.md` — About Page (~5622 tok)

## .claude/skills/bundle/content-copywriting/article-rewriter/

- `SKILL.md` — Article Rewriter (~1752 tok)

## .claude/skills/bundle/content-copywriting/authority-content-strategy/

- `SKILL.md` — Authority Content Strategy (~2404 tok)

## .claude/skills/bundle/content-copywriting/blog-post/

- `SKILL.md` — Blog Post (~3778 tok)

## .claude/skills/bundle/content-copywriting/book-outline/

- `SKILL.md` — Book Outline Builder (~1661 tok)

## .claude/skills/bundle/content-copywriting/book-proposal/

- `SKILL.md` — Book Proposal (~2248 tok)

## .claude/skills/bundle/content-copywriting/brand-story/

- `SKILL.md` — Brand Story Builder (~1838 tok)

## .claude/skills/bundle/content-copywriting/caption-writer/

- `SKILL.md` — Caption Writer (~1407 tok)

## .claude/skills/bundle/content-copywriting/case-study/

- `SKILL.md` — Case Study (~4611 tok)

## .claude/skills/bundle/content-copywriting/collection-page-copy/

- `SKILL.md` — Collection Page Copy (~1642 tok)

## .claude/skills/bundle/content-copywriting/comparison-article/

- `SKILL.md` — Comparison Article (~1830 tok)

## .claude/skills/bundle/content-copywriting/content-audit/

- `SKILL.md` — Content Audit (~1791 tok)

## .claude/skills/bundle/content-copywriting/content-brief/

- `SKILL.md` — Content Brief Builder (~1838 tok)

## .claude/skills/bundle/content-copywriting/content-calendar/

- `SKILL.md` — Content Calendar (~5214 tok)

## .claude/skills/bundle/content-copywriting/content-cluster-plan/

- `SKILL.md` — Content Cluster Plan (~1572 tok)

## .claude/skills/bundle/content-copywriting/content-gap-finder/

- `SKILL.md` — Content Gap Finder (~1789 tok)

## .claude/skills/bundle/content-copywriting/content-pillar-strategy/

- `SKILL.md` — Content Pillar Strategy (~1868 tok)

## .claude/skills/bundle/content-copywriting/content-repurpose/

- `README.md` — Project documentation (~741 tok)
- `SKILL.md` — Content Repurpose (~1543 tok)

## .claude/skills/bundle/content-copywriting/content-style-guide/

- `SKILL.md` — Content Style Guide (~2042 tok)

## .claude/skills/bundle/content-copywriting/donation-page-copy/

- `SKILL.md` — Donation Page Copy (~1788 tok)

## .claude/skills/bundle/content-copywriting/ebook-outline/

- `SKILL.md` — Ebook Outline (~1856 tok)

## .claude/skills/bundle/content-copywriting/expert-roundup-pitch/

- `SKILL.md` — Expert Roundup Pitch (~1527 tok)

## .claude/skills/bundle/content-copywriting/faq-generator/

- `SKILL.md` — FAQ Generator (~6419 tok)

## .claude/skills/bundle/content-copywriting/feature-announcement/

- `SKILL.md` — Feature Announcement (~1580 tok)

## .claude/skills/bundle/content-copywriting/ghostwriter-brief/

- `SKILL.md` — Ghostwriter Brief (~1981 tok)

## .claude/skills/bundle/content-copywriting/headline-generator/

- `SKILL.md` — Headline Generator (~1772 tok)

## .claude/skills/bundle/content-copywriting/help-center-article/

- `SKILL.md` — Help Center Article (~1670 tok)

## .claude/skills/bundle/content-copywriting/hook-generator/

- `SKILL.md` — Hook Generator (~1638 tok)

## .claude/skills/bundle/content-copywriting/landing-page-copy/

- `SKILL.md` — Landing Page Copy (~1993 tok)

## .claude/skills/bundle/content-copywriting/linkedin-article/

- `SKILL.md` — LinkedIn Article (~1773 tok)

## .claude/skills/bundle/content-copywriting/listicle-generator/

- `SKILL.md` — Listicle Generator (~1713 tok)

## .claude/skills/bundle/content-copywriting/microcopy-writer/

- `SKILL.md` — Microcopy Writer (~1837 tok)

## .claude/skills/bundle/content-copywriting/newsletter-builder/

- `SKILL.md` — Newsletter Builder (~3410 tok)

## .claude/skills/bundle/content-copywriting/newsletter-strategy/

- `SKILL.md` — Newsletter Strategy (~2016 tok)

## .claude/skills/bundle/content-copywriting/pillar-page/

- `SKILL.md` — Pillar Page (~1879 tok)

## .claude/skills/bundle/content-copywriting/podcast-one-sheet/

- `SKILL.md` — Podcast One-Sheet (~1701 tok)

## .claude/skills/bundle/content-copywriting/podcast-show-notes/

- `SKILL.md` — Podcast Show Notes (~2572 tok)

## .claude/skills/bundle/content-copywriting/press-kit/

- `SKILL.md` — Press Kit (~1657 tok)

## .claude/skills/bundle/content-copywriting/press-release/

- `SKILL.md` — Press Release (~6123 tok)

## .claude/skills/bundle/content-copywriting/pricing-page-copy/

- `SKILL.md` — Pricing Page Copy (~1693 tok)

## .claude/skills/bundle/content-copywriting/product-changelog/

- `SKILL.md` — Product Changelog (~1717 tok)

## .claude/skills/bundle/content-copywriting/product-description/

- `SKILL.md` — Product Description (~4253 tok)

## .claude/skills/bundle/content-copywriting/product-faq/

- `SKILL.md` — Product FAQ (~1571 tok)

## .claude/skills/bundle/content-copywriting/professional-bio/

- `SKILL.md` — Professional Bio (~2211 tok)

## .claude/skills/bundle/content-copywriting/recipe-card/

- `SKILL.md` — Recipe Card (~2151 tok)

## .claude/skills/bundle/content-copywriting/release-notes/

- `SKILL.md` — Release Notes (~1843 tok)

## .claude/skills/bundle/content-copywriting/script-to-blog/

- `SKILL.md` — Script to Blog (~1849 tok)

## .claude/skills/bundle/content-copywriting/seo-content-brief/

- `SKILL.md` — SEO Content Brief (~1518 tok)

## .claude/skills/bundle/content-copywriting/speech-writer/

- `SKILL.md` — Speech Writer (~1746 tok)

## .claude/skills/bundle/content-copywriting/thought-leader-content-plan/

- `SKILL.md` — Thought Leader Content Plan (~2008 tok)

## .claude/skills/bundle/content-copywriting/thought-leadership/

- `SKILL.md` — Thought Leadership (~1877 tok)

## .claude/skills/bundle/content-copywriting/tutorial-writer/

- `SKILL.md` — Tutorial Writer (~2944 tok)

## .claude/skills/bundle/content-copywriting/video-script/

- `SKILL.md` — Video Script (~4178 tok)

## .claude/skills/bundle/content-copywriting/viral-content-formula/

- `SKILL.md` — Viral Content Formula (~2224 tok)

## .claude/skills/bundle/content-copywriting/white-paper/

- `SKILL.md` — White Paper (~2129 tok)

## .claude/skills/bundle/content-copywriting/youtube-thumbnail/

- `SKILL.md` — YouTube Thumbnail Generator (~4280 tok)

## .claude/skills/bundle/e-commerce-products/cross-border-selling/

- `SKILL.md` — Cross-Border Selling (~1752 tok)

## .claude/skills/bundle/e-commerce-products/dropshipping-supplier-brief/

- `SKILL.md` — Dropshipping Supplier Brief (~1593 tok)

## .claude/skills/bundle/e-commerce-products/exchange-policy/

- `SKILL.md` — Exchange Policy (~1703 tok)

## .claude/skills/bundle/e-commerce-products/food-delivery-strategy/

- `SKILL.md` — Food Delivery Strategy (~2124 tok)

## .claude/skills/bundle/e-commerce-products/food-truck-business-plan/

- `SKILL.md` — Food Truck Business Plan (~2094 tok)

## .claude/skills/bundle/e-commerce-products/gift-guide/

- `SKILL.md` — Gift Guide (~1415 tok)

## .claude/skills/bundle/e-commerce-products/investment-property-analysis/

- `SKILL.md` — Investment Property Analysis (~1635 tok)

## .claude/skills/bundle/e-commerce-products/lease-agreement-checklist/

- `SKILL.md` — Lease Agreement Checklist (~1700 tok)

## .claude/skills/bundle/e-commerce-products/marketplace-listing/

- `SKILL.md` — Marketplace Listing (~1685 tok)

## .claude/skills/bundle/e-commerce-products/neighborhood-guide/

- `SKILL.md` — Neighborhood Guide (~1809 tok)

## .claude/skills/bundle/e-commerce-products/open-house-plan/

- `SKILL.md` — Open House Plan (~1719 tok)

## .claude/skills/bundle/e-commerce-products/product-recall-plan/

- `SKILL.md` — Product Recall Plan (~1761 tok)

## .claude/skills/bundle/e-commerce-products/product-sourcing-brief/

- `SKILL.md` — Product Sourcing Brief (~1662 tok)

## .claude/skills/bundle/e-commerce-products/property-listing/

- `SKILL.md` — Property Listing (~1608 tok)

## .claude/skills/bundle/e-commerce-products/property-management-sop/

- `SKILL.md` — Property Management SOP (~2048 tok)

## .claude/skills/bundle/e-commerce-products/real-estate-crm-setup/

- `SKILL.md` — Real Estate CRM Setup (~1940 tok)

## .claude/skills/bundle/e-commerce-products/real-estate-newsletter/

- `SKILL.md` — Real Estate Newsletter (~1738 tok)

## .claude/skills/bundle/e-commerce-products/rental-listing/

- `SKILL.md` — Rental Listing (~1636 tok)

## .claude/skills/bundle/e-commerce-products/seasonal-inventory-plan/

- `SKILL.md` — Seasonal Inventory Plan (~1641 tok)

## .claude/skills/bundle/e-commerce-products/seller-onboarding/

- `SKILL.md` — Seller Onboarding (~2040 tok)

## .claude/skills/bundle/e-commerce-products/size-guide/

- `SKILL.md` — Size Guide (~1442 tok)

## .claude/skills/bundle/e-commerce-products/store-launch-plan/

- `SKILL.md` — Store Launch Plan (~1625 tok)

## .claude/skills/bundle/e-commerce-products/subscription-box-plan/

- `SKILL.md` — Subscription Box Plan (~1770 tok)

## .claude/skills/bundle/e-commerce-products/wholesale-catalog/

- `SKILL.md` — Wholesale Catalog (~1460 tok)

## .claude/skills/bundle/email-marketing-automation/abandoned-cart-email/

- `SKILL.md` — Abandoned Cart Email (~2231 tok)

## .claude/skills/bundle/email-marketing-automation/ambassador-program/

- `SKILL.md` — Ambassador Program (~2343 tok)

## .claude/skills/bundle/email-marketing-automation/black-friday-emails/

- `SKILL.md` — Black Friday Emails (~1390 tok)

## .claude/skills/bundle/email-marketing-automation/cart-recovery-sms/

- `SKILL.md` — Cart Recovery SMS (~1376 tok)

## .claude/skills/bundle/email-marketing-automation/cause-marketing-campaign/

- `SKILL.md` — Cause Marketing Campaign (~1827 tok)

## .claude/skills/bundle/email-marketing-automation/cold-outreach/

- `SKILL.md` — Cold Outreach (~3853 tok)

## .claude/skills/bundle/email-marketing-automation/customer-review-strategy/

- `SKILL.md` — Customer Review Strategy (~1652 tok)

## .claude/skills/bundle/email-marketing-automation/downsell-sequence/

- `SKILL.md` — Downsell Sequence (~1668 tok)

## .claude/skills/bundle/email-marketing-automation/drip-campaign/

- `SKILL.md` — Drip Campaign (~1744 tok)

## .claude/skills/bundle/email-marketing-automation/email-ab-test-plan/

- `SKILL.md` — Email A/B Test Plan (~1558 tok)

## .claude/skills/bundle/email-marketing-automation/email-deliverability-audit/

- `SKILL.md` — Email Deliverability Audit (~2728 tok)

## .claude/skills/bundle/email-marketing-automation/email-design-system/

- `SKILL.md` — Email Design System (~1540 tok)

## .claude/skills/bundle/email-marketing-automation/email-list-cleanup/

- `SKILL.md` — Email List Cleanup (~1514 tok)

## .claude/skills/bundle/email-marketing-automation/email-newsletter-template/

- `SKILL.md` — Email Newsletter Template (~2186 tok)

## .claude/skills/bundle/email-marketing-automation/email-preference-center/

- `SKILL.md` — Email Preference Center (~1383 tok)

## .claude/skills/bundle/email-marketing-automation/email-sequence/

- `SKILL.md` — Email Sequence (~5002 tok)

## .claude/skills/bundle/email-marketing-automation/email-subject-line-tester/

- `SKILL.md` — Email Subject Line Tester (~1478 tok)

## .claude/skills/bundle/email-marketing-automation/engagement-playbook/

- `SKILL.md` — Engagement Playbook (~2259 tok)

## .claude/skills/bundle/email-marketing-automation/flash-sale-campaign/

- `SKILL.md` — Flash Sale Campaign (~1380 tok)

## .claude/skills/bundle/email-marketing-automation/launch-email-sequence/

- `SKILL.md` — Launch Email Sequence (~1430 tok)

## .claude/skills/bundle/email-marketing-automation/loyalty-program/

- `SKILL.md` — Loyalty Program Designer (~1759 tok)

## .claude/skills/bundle/email-marketing-automation/milestone-email/

- `SKILL.md` — Milestone Email (~1439 tok)

## .claude/skills/bundle/email-marketing-automation/order-bump-copy/

- `SKILL.md` — Order Bump Copy (~1577 tok)

## .claude/skills/bundle/email-marketing-automation/payment-plan-offer/

- `SKILL.md` — Payment Plan Offer (~1504 tok)

## .claude/skills/bundle/email-marketing-automation/price-increase-notice/

- `SKILL.md` — Price Increase Notice (~1622 tok)

## .claude/skills/bundle/email-marketing-automation/product-launch-email/

- `SKILL.md` — Product Launch Email (~2039 tok)

## .claude/skills/bundle/email-marketing-automation/re-engagement-email/

- `SKILL.md` — Re-Engagement Email (~2188 tok)

## .claude/skills/bundle/email-marketing-automation/referral-program/

- `SKILL.md` — Referral Program Designer (~1699 tok)

## .claude/skills/bundle/email-marketing-automation/renewal-campaign/

- `SKILL.md` — Renewal Campaign (~1514 tok)

## .claude/skills/bundle/email-marketing-automation/seasonal-campaign/

- `SKILL.md` — Seasonal Campaign (~3134 tok)

## .claude/skills/bundle/email-marketing-automation/social-proof-collector/

- `SKILL.md` — Social Proof Collector (~2333 tok)

## .claude/skills/bundle/email-marketing-automation/testimonial-collector/

- `SKILL.md` — Testimonial Collector (~2031 tok)

## .claude/skills/bundle/email-marketing-automation/thank-you-campaign/

- `SKILL.md` — Thank-You Campaign (~2521 tok)

## .claude/skills/bundle/email-marketing-automation/transactional-email/

- `SKILL.md` — Transactional Email (~2192 tok)

## .claude/skills/bundle/email-marketing-automation/trial-to-paid-email/

- `SKILL.md` — Trial-to-Paid Email Sequence (~1916 tok)

## .claude/skills/bundle/email-marketing-automation/two-sided-email-strategy/

- `SKILL.md` — Two-Sided Email Strategy (~2097 tok)

## .claude/skills/bundle/email-marketing-automation/upsell-sequence/

- `SKILL.md` — Upsell Sequence (~5388 tok)

## .claude/skills/bundle/email-marketing-automation/user-generated-content/

- `SKILL.md` — User-Generated Content (~1979 tok)

## .claude/skills/bundle/email-marketing-automation/waitlist-builder/

- `SKILL.md` — Waitlist Builder (~1569 tok)

## .claude/skills/bundle/email-marketing-automation/webinar-email-sequence/

- `SKILL.md` — Webinar Email Sequence (~1348 tok)

## .claude/skills/bundle/email-marketing-automation/welcome-sequence/

- `SKILL.md` — Welcome Sequence Builder (~1864 tok)

## .claude/skills/bundle/email-marketing-automation/win-back-campaign/

- `SKILL.md` — Win-Back Campaign (~5039 tok)

## .claude/skills/bundle/sales-funnels/business-plan/

- `SKILL.md` — Business Plan Writer (~1728 tok)

## .claude/skills/bundle/sales-funnels/coaching-framework/

- `SKILL.md` — Coaching Framework (~3100 tok)

## .claude/skills/bundle/sales-funnels/collaboration-agreement/

- `SKILL.md` — Collaboration Agreement (~2045 tok)

## .claude/skills/bundle/sales-funnels/competitor-analysis/

- `SKILL.md` — Competitor Analysis (~6825 tok)

## .claude/skills/bundle/sales-funnels/customer-journey-map/

- `SKILL.md` — Customer Journey Map (~3209 tok)

## .claude/skills/bundle/sales-funnels/customer-persona/

- `SKILL.md` — Customer Persona (~1679 tok)

## .claude/skills/bundle/sales-funnels/customer-segmentation/

- `SKILL.md` — Customer Segmentation (~1933 tok)

## .claude/skills/bundle/sales-funnels/discovery-call-script/

- `SKILL.md` — Discovery Call Script (~1930 tok)

## .claude/skills/bundle/sales-funnels/joint-venture-proposal/

- `SKILL.md` — Joint Venture Proposal (~1841 tok)

## .claude/skills/bundle/sales-funnels/lead-magnet/

- `SKILL.md` — Lead Magnet (~5092 tok)

## .claude/skills/bundle/sales-funnels/market-research/

- `SKILL.md` — Market Research (~1732 tok)

## .claude/skills/bundle/sales-funnels/market-sizing/

- `SKILL.md` — Market Sizing (~1457 tok)

## .claude/skills/bundle/sales-funnels/membership-site-plan/

- `SKILL.md` — Membership Site Plan (~3357 tok)

## .claude/skills/bundle/sales-funnels/objection-handler/

- `SKILL.md` — Objection Handler Playbook (~1926 tok)

## .claude/skills/bundle/sales-funnels/partnership-proposal/

- `SKILL.md` — Partnership Proposal (~2471 tok)

## .claude/skills/bundle/sales-funnels/pitch-deck/

- `SKILL.md` — Pitch Deck (~3802 tok)

## .claude/skills/bundle/sales-funnels/product-comparison/

- `SKILL.md` — Product Comparison (~2977 tok)

## .claude/skills/bundle/sales-funnels/product-roadmap/

- `SKILL.md` — Product Roadmap (~4362 tok)

## .claude/skills/bundle/sales-funnels/proposal-writer/

- `SKILL.md` — Proposal Writer (~5255 tok)

## .claude/skills/bundle/sales-funnels/revenue-model/

- `SKILL.md` — Revenue Model (~1452 tok)

## .claude/skills/bundle/sales-funnels/sales-battlecard/

- `SKILL.md` — Sales Battlecard (~1585 tok)

## .claude/skills/bundle/sales-funnels/sales-deck/

- `SKILL.md` — Sales Deck (~1602 tok)

## .claude/skills/bundle/sales-funnels/sales-email-template/

- `SKILL.md` — Sales Email Template (~1588 tok)

## .claude/skills/bundle/sales-funnels/sales-funnel-builder/

- `SKILL.md` — Sales Funnel Builder (~1610 tok)

## .claude/skills/bundle/sales-funnels/sales-script/

- `SKILL.md` — Sales Script Builder (~1767 tok)

## .claude/skills/bundle/sales-funnels/strategic-alliance-plan/

- `SKILL.md` — Strategic Alliance Plan (~1846 tok)

## .claude/skills/bundle/sales-funnels/swot-analysis/

- `SKILL.md` — SWOT Analysis (~7119 tok)

## .claude/skills/bundle/sales-funnels/value-proposition-canvas/

- `SKILL.md` — Value Proposition Canvas (~1546 tok)

## .claude/skills/bundle/sales-funnels/webinar-sales-script/

- `SKILL.md` — Webinar Sales Script (~1626 tok)

## .claude/skills/bundle/sales-funnels/win-loss-analysis/

- `SKILL.md` — Win/Loss Analysis (~1599 tok)

## .claude/skills/bundle/seo-search/checkout-optimizer/

- `SKILL.md` — Checkout Optimizer (~2443 tok)

## .claude/skills/bundle/seo-search/featured-snippet-optimizer/

- `SKILL.md` — Featured Snippet Optimizer (~1675 tok)

## .claude/skills/bundle/seo-search/google-business-profile/

- `SKILL.md` — Google Business Profile (~1859 tok)

## .claude/skills/bundle/seo-search/keyword-research/

- `SKILL.md` — Keyword Research (~1573 tok)

## .claude/skills/bundle/seo-search/landing-page-audit/

- `SKILL.md` — Landing Page Audit (~1668 tok)

## .claude/skills/bundle/seo-search/link-building-plan/

- `SKILL.md` — Link Building Plan (~1649 tok)

## .claude/skills/bundle/seo-search/local-seo-plan/

- `SKILL.md` — Local SEO Plan (~1674 tok)

## .claude/skills/bundle/seo-search/marketplace-seo/

- `SKILL.md` — Marketplace SEO (~2136 tok)

## .claude/skills/bundle/seo-search/meta-tag-optimizer/

- `SKILL.md` — Meta Tag Optimizer (~1568 tok)

## .claude/skills/bundle/seo-search/platform-migration/

- `SKILL.md` — Platform Migration (~2164 tok)

## .claude/skills/bundle/seo-search/product-listing-optimizer/

- `SKILL.md` — Product Listing Optimizer (~1677 tok)

## .claude/skills/bundle/seo-search/schema-markup-guide/

- `SKILL.md` — Schema Markup Guide (~1626 tok)

## .claude/skills/bundle/seo-search/seo-audit/

- `SKILL.md` — SEO Audit (~1697 tok)

## .claude/skills/bundle/seo-search/seo-competitor-analysis/

- `SKILL.md` — SEO Competitor Analysis (~1630 tok)

## .claude/skills/bundle/seo-search/seo-migration-plan/

- `SKILL.md` — SEO Migration Plan (~1751 tok)

## .claude/skills/bundle/seo-search/seo-reporting-template/

- `SKILL.md` — SEO Reporting Template (~1646 tok)

## .claude/skills/bundle/seo-search/site-architecture-plan/

- `SKILL.md` — Site Architecture Plan (~1657 tok)

## .claude/skills/bundle/seo-search/store-page-audit/

- `SKILL.md` — Store Page Audit (~1648 tok)

## .claude/skills/bundle/seo-search/technical-seo-checklist/

- `SKILL.md` — Technical SEO Checklist (~1800 tok)

## .claude/skills/bundle/seo-search/youtube-seo/

- `SKILL.md` — YouTube SEO (~1679 tok)

## .claude/skills/bundle/social-media/author-platform-plan/

- `SKILL.md` — Author Platform Plan (~2050 tok)

## .claude/skills/bundle/social-media/brand-photography-brief/

- `SKILL.md` — Brand Photography Brief (~3008 tok)

## .claude/skills/bundle/social-media/co-marketing-plan/

- `SKILL.md` — Co-Marketing Plan (~1927 tok)

## .claude/skills/bundle/social-media/community-launch/

- `SKILL.md` — Community Launch (~2106 tok)

## .claude/skills/bundle/social-media/community-moderation/

- `SKILL.md` — Community Moderation (~2406 tok)

## .claude/skills/bundle/social-media/crisis-comms/

- `SKILL.md` — Crisis Communications Planner (~1683 tok)

## .claude/skills/bundle/social-media/facebook-group-plan/

- `SKILL.md` — Facebook Group Plan (~2307 tok)

## .claude/skills/bundle/social-media/food-photography-brief/

- `SKILL.md` — Food Photography Brief (~2148 tok)

## .claude/skills/bundle/social-media/guest-post-pitch/

- `SKILL.md` — Guest Post Pitch (~2005 tok)

## .claude/skills/bundle/social-media/hashtag-strategy/

- `SKILL.md` — Hashtag Strategy (~3184 tok)

## .claude/skills/bundle/social-media/influencer-campaign-brief/

- `SKILL.md` — Influencer Campaign Brief (~1726 tok)

## .claude/skills/bundle/social-media/influencer-outreach/

- `SKILL.md` — Influencer Outreach (~2148 tok)

## .claude/skills/bundle/social-media/instagram-carousel/

- `SKILL.md` — Instagram Carousel (~1789 tok)

## .claude/skills/bundle/social-media/linkedin-profile-optimizer/

- `SKILL.md` — LinkedIn Profile Optimizer (~2233 tok)

## .claude/skills/bundle/social-media/linkedin-strategy/

- `SKILL.md` — LinkedIn Strategy (~2120 tok)

## .claude/skills/bundle/social-media/media-kit/

- `SKILL.md` — Media Kit Builder (~1287 tok)

## .claude/skills/bundle/social-media/meme-content-brief/

- `SKILL.md` — Meme Content Brief (~1680 tok)

## .claude/skills/bundle/social-media/menu-design-brief/

- `SKILL.md` — Menu Design Brief (~2134 tok)

## .claude/skills/bundle/social-media/networking-strategy/

- `SKILL.md` — Networking Strategy (~1861 tok)

## .claude/skills/bundle/social-media/personal-brand-strategy/

- `SKILL.md` — Personal Brand Strategy (~1952 tok)

## .claude/skills/bundle/social-media/pinterest-strategy/

- `SKILL.md` — Pinterest Strategy (~2051 tok)

## .claude/skills/bundle/social-media/platform-community-guidelines/

- `SKILL.md` — Platform Community Guidelines (~2101 tok)

## .claude/skills/bundle/social-media/podcast-guest-pitch/

- `SKILL.md` — Podcast Guest Pitch (~2040 tok)

## .claude/skills/bundle/social-media/pr-pitch/

- `SKILL.md` — PR Pitch Writer (~1388 tok)

## .claude/skills/bundle/social-media/product-photography-brief/

- `SKILL.md` — Product Photography Brief (~3531 tok)

## .claude/skills/bundle/social-media/reddit-strategy/

- `SKILL.md` — Reddit Strategy (~2024 tok)

## .claude/skills/bundle/social-media/short-form-video-plan/

- `SKILL.md` — Short-Form Video Plan (~1911 tok)

## .claude/skills/bundle/social-media/social-listening-plan/

- `SKILL.md` — Social Listening Plan (~2101 tok)

## .claude/skills/bundle/social-media/social-media-audit/

- `SKILL.md` — Social Media Audit (~6858 tok)

## .claude/skills/bundle/social-media/social-media-calendar/

- `SKILL.md` — Social Media Calendar (~2041 tok)

## .claude/skills/bundle/social-media/social-media-graphics/

- `SKILL.md` — Social Media Graphics (~4494 tok)

## .claude/skills/bundle/social-media/social-media-policy/

- `SKILL.md` — Social Media Policy (~2201 tok)

## .claude/skills/bundle/social-media/social-media-strategy/

- `SKILL.md` — Social Media Strategy (~1985 tok)

## .claude/skills/bundle/social-media/sponsor-pitch/

- `SKILL.md` — Sponsor Pitch (~1587 tok)

## .claude/skills/bundle/social-media/thread-hook-writer/

- `SKILL.md` — Thread Hook Writer (~1942 tok)

## .claude/skills/bundle/social-media/tiktok-script/

- `SKILL.md` — TikTok Script (~1848 tok)

## .claude/skills/bundle/social-media/twitter-thread/

- `SKILL.md` — Twitter Thread (~1748 tok)

## .claude/skills/bundle/social-media/youtube-strategy/

- `SKILL.md` — YouTube Strategy (~2135 tok)

## .claude/skills/claude-api/

- `SKILL.md` — Claude API (~2123 tok)

## .claude/skills/claude-api/agents/

- `openai.yaml` (~80 tok)

## .claude/skills/coding-standards/

- `SKILL.md` — Coding Standards & Best Practices (~2889 tok)

## .claude/skills/coding-standards/agents/

- `openai.yaml` — Declares safety (~75 tok)

## .claude/skills/content-engine/

- `SKILL.md` — Content Engine (~643 tok)

## .claude/skills/content-engine/agents/

- `openai.yaml` (~79 tok)

## .claude/skills/continuous-learning/

- `config.json` (~112 tok)
- `evaluate-session.sh` — Continuous Learning - Session Evaluator (~576 tok)
- `SKILL.md` — Continuous Learning Skill (~215 tok)

## .claude/skills/crosspost/

- `SKILL.md` — Crosspost (~1337 tok)

## .claude/skills/crosspost/agents/

- `openai.yaml` (~86 tok)

## .claude/skills/deep-research/

- `SKILL.md` — Deep Research (~1143 tok)

## .claude/skills/deep-research/agents/

- `openai.yaml` (~81 tok)

## .claude/skills/dmux-workflows/

- `SKILL.md` — dmux Workflows (~1179 tok)

## .claude/skills/dmux-workflows/agents/

- `openai.yaml` (~72 tok)

## .claude/skills/documentation-lookup/

- `SKILL.md` — Documentation Lookup (Context7) (~1225 tok)

## .claude/skills/documentation-lookup/agents/

- `openai.yaml` (~69 tok)

## .claude/skills/e2e-testing/

- `SKILL.md` — E2E Testing Patterns (~1985 tok)

## .claude/skills/e2e-testing/agents/

- `openai.yaml` (~67 tok)

## .claude/skills/eval-harness/

- `SKILL.md` — Eval Harness Skill (~1379 tok)

## .claude/skills/eval-harness/agents/

- `openai.yaml` (~73 tok)

## .claude/skills/everything-claude-code/

- `SKILL.md` — Everything Claude Code Conventions (~2794 tok)

## .claude/skills/everything-claude-code/agents/

- `openai.yaml` (~89 tok)

## .claude/skills/exa-search/

- `SKILL.md` — Exa Search (~1203 tok)

## .claude/skills/exa-search/agents/

- `openai.yaml` (~79 tok)

## .claude/skills/fal-ai-media/

- `SKILL.md` — fal.ai Media Generation (~1707 tok)

## .claude/skills/fal-ai-media/agents/

- `openai.yaml` (~73 tok)

## .claude/skills/frontend-design/

- `SKILL.md` — Frontend Design — Full-Stack Theme & UI/UX Architecture (~5004 tok)

## .claude/skills/frontend-patterns/

- `SKILL.md` — Frontend Development Patterns (~3687 tok)

## .claude/skills/frontend-patterns/agents/

- `openai.yaml` (~72 tok)

## .claude/skills/frontend-slides/

- `SKILL.md` — Frontend Slides (~1640 tok)
- `STYLE_PRESETS.md` — Style Presets Reference (~2287 tok)

## .claude/skills/frontend-slides/agents/

- `openai.yaml` (~82 tok)

## .claude/skills/immersive-architect/

- `SKILL.md` — Immersive Interactive Architect (~3989 tok)

## .claude/skills/immersive-interactive-architect/

- `SKILL.md` — Immersive Interactive Architect (~3989 tok)

## .claude/skills/interactive-web-development/

- `SKILL.md` — Interactive Web Development (~6197 tok)

## .claude/skills/investor-materials/

- `SKILL.md` — Investor Materials (~681 tok)

## .claude/skills/investor-materials/agents/

- `openai.yaml` (~86 tok)

## .claude/skills/investor-outreach/

- `SKILL.md` — Investor Outreach (~535 tok)

## .claude/skills/investor-outreach/agents/

- `openai.yaml` (~86 tok)

## .claude/skills/learned/

- `devskyy-analytics-patterns.md` — DevSkyy Analytics Patterns (~265 tok)

## .claude/skills/market-research/

- `SKILL.md` — Market Research (~557 tok)

## .claude/skills/market-research/agents/

- `openai.yaml` (~81 tok)

## .claude/skills/mcp-server-patterns/

- `SKILL.md` — MCP Server Patterns (~965 tok)

## .claude/skills/nextjs-turbopack/

- `SKILL.md` — Next.js and Turbopack (~540 tok)

## .claude/skills/nextjs-turbopack/agents/

- `openai.yaml` (~70 tok)

## .claude/skills/python-patterns/

- `SKILL.md` — Python Patterns (~1620 tok)

## .claude/skills/search-first/

- `SKILL.md` — /search-first — Research Before You Code (~1417 tok)

## .claude/skills/security-review/

- `SKILL.md` — Security Review Skill (~3049 tok)

## .claude/skills/security-review/agents/

- `openai.yaml` (~83 tok)

## .claude/skills/skyyrose-brand-dna/

- `SKILL.md` — SkyyRose Brand DNA (~1574 tok)

## .claude/skills/skyyrose-content-engine/

- `SKILL.md` — SkyyRose Content Engine (~2511 tok)

## .claude/skills/skyyrose-email-flows/

- `SKILL.md` — SkyyRose Email Marketing System (~3730 tok)

## .claude/skills/skyyrose-launch-commander/

- `SKILL.md` — SkyyRose Launch Commander (~2201 tok)

## .claude/skills/skyyrose-paid-media/

- `SKILL.md` — SkyyRose Paid Media System (~3047 tok)

## .claude/skills/skyyrose-product-copy/

- `SKILL.md` — SkyyRose Product Copy System (~3318 tok)

## .claude/skills/skyyrose-seo-commerce/

- `SKILL.md` — SkyyRose E-Commerce SEO System (~3685 tok)

## .claude/skills/strategic-compact/

- `SKILL.md` — Strategic Compact Skill (~1000 tok)
- `suggest-compact.sh` — Strategic Compact Suggester (~467 tok)

## .claude/skills/strategic-compact/agents/

- `openai.yaml` (~74 tok)

## .claude/skills/tdd-workflow/

- `SKILL.md` — Test-Driven Development Workflow (~2409 tok)

## .claude/skills/tdd-workflow/agents/

- `openai.yaml` (~74 tok)

## .claude/skills/token-aware-behavior/

- `SKILL.md` — Token-Aware Behavior (~1481 tok)

## .claude/skills/ui-ux-pro-max/

- `SKILL.md` — UI/UX Pro Max - Design Intelligence (~3521 tok)

## .claude/skills/ui-ux-pro-max/data/

- `charts.csv` (~2013 tok)
- `colors.csv` (~2591 tok)
- `icons.csv` (~3570 tok)
- `landing.csv` (~3837 tok)
- `products.csv` (~7944 tok)
- `react-performance.csv` — Exports getUser (~3953 tok)
- `styles.csv` — Declares hierarchy (~25313 tok)
- `typography.csv` (~8501 tok)
- `ui-reasoning.csv` (~8286 tok)
- `ux-guidelines.csv` (~5005 tok)
- `web-interface.csv` — Declares email (~1993 tok)

## .claude/skills/ui-ux-pro-max/data/stacks/

- `astro.csv` — Exports prerender (~3165 tok)
- `flutter.csv` — Declares MyWidget (~2792 tok)
- `html-tailwind.csv` — /*.{js,ts,jsx,tsx}']",purge: [...],High,https://tailwindcss.com/docs/content-configuration (~3030 tok)
- `jetpack-compose.csv` — Declares Route (~2185 tok)
- `nextjs.csv` — Exports Page (~3346 tok)
- `nuxt-ui.csv` — Declares overlay (~3736 tok)
- `nuxtjs.csv` — in: submit, submit (~4411 tok)
- `react-native.csv` — Declares App (~2676 tok)
- `react.csv` — total: User, User, Button + 3 more (~3471 tok)
- `shadcn.csv` — Declares styles (~4246 tok)
- `svelte.csv` — Exports load (~2951 tok)
- `swiftui.csv` — Declares MyView (~2900 tok)
- `vue.csv` — Exports useFetch (~2949 tok)

## .claude/skills/ui-ux-pro-max/scripts/

- `core.py` — -*- coding: utf-8 -*- (~2922 tok)
- `design_system.py` — -*- coding: utf-8 -*- (~12449 tok)
- `search.py` — -*- coding: utf-8 -*- (~1563 tok)

## .claude/skills/verification-loop/

- `SKILL.md` — Verification Loop Skill (~623 tok)

## .claude/skills/verification-loop/agents/

- `openai.yaml` (~74 tok)

## .claude/skills/wordpress-woocommerce-automation/

- `SKILL.md` — WordPress WooCommerce Automation (~1760 tok)

## .claude/skills/wordpress-woocommerce-automation/references/

- `elementor-json.md` — Elementor JSON Patterns (~1742 tok)

## .claude/skills/wp-deploy/

- `SKILL.md` — WordPress Theme Deploy (~613 tok)

## .claude/skills/wp-playground/

- `.openskills.json` (~58 tok)
- `SKILL.md` — WordPress Playground (~866 tok)

## .claude/skills/wp-playground/scripts/

- `start-server.sh` — Start WordPress Playground server and wait for it to be ready (~511 tok)
- `stop-server.sh` — Stop WordPress Playground server by PID (~190 tok)

## .claude/skills/x-api/

- `SKILL.md` — X API (~1387 tok)

## .claude/skills/x-api/agents/

- `openai.yaml` (~80 tok)

## .claude/worktrees/inspiring-lewin/

- `.dockerignore` — Docker ignore rules (~122 tok)
- `.eslintrc.cjs` — ESLint configuration (~954 tok)
- `.gitattributes` — Git attributes (~70 tok)
- `.gitignore` — Git ignore rules (~1303 tok)
- `.markdownlint.json` (~57 tok)
- `.npmrc` (~6 tok)
- `.nvmrc` (~1 tok)
- `.pre-commit-config.yaml` — Pre-commit Configuration - Optimized for Speed and Safety (~1529 tok)
- `.prettierrc.js` (~381 tok)
- `.vercelignore` — DevSkyy .vercelignore - Production Deployment (~444 tok)
- `alembic.ini` — Alembic configuration for DevSkyy (~163 tok)
- `autotrain_config.yaml` (~131 tok)
- `CLAUDE.md` — DevSkyy — Claude Code Configuration (~3404 tok)
- `conftest.py` — /*.py", (~500 tok)
- `deployment_summary.json` (~406 tok)
- `devskyy_mcp.py` (~1153 tok)
- `docker-compose.staging.yml` — Docker Compose: 14 services (~3431 tok)
- `docker-compose.yml` — Docker Compose services (~1549 tok)
- `docker-entrypoint.sh` — Enterprise startup script with error handling and logging (~1012 tok)
- `Dockerfile` — Docker container definition (~1158 tok)
- `Dockerfile.worker` — DevSkyy Worker - Lightweight build for background task processing (~813 tok)
- `fastmcp.config.json` (~287 tok)
- `fly.toml` — fly.toml - DevSkyy Enterprise Platform (~1070 tok)
- `init.sql` — DevSkyy Database Initialization (~1088 tok)
- `lint-staged.config.mjs` — Declares relPaths (~346 tok)
- `main_enterprise.py` — API: GET (4 endpoints) (~3477 tok)
- `Makefile` — Make build targets (~1530 tok)
- `mypy.ini` (~383 tok)
- `nginx-app.devskyy.app.conf` — Nginx configuration for app.devskyy.app (self-hosted alternative) (~1497 tok)
- `nginx.conf` — Nginx configuration (~1630 tok)
- `package.json` — Node.js package manifest (~2968 tok)
- `pnpm-lock.yaml` — pnpm lock file (~195082 tok)
- `pre-build-check.sh` — DevSkyy Pre-Build Verification (~2307 tok)
- `prometheus.yml` — DevSkyy - Prometheus Configuration (~790 tok)
- `pyproject.toml` — Python project configuration (~3866 tok)
- `README.md` — Project documentation (~1441 tok)
- `requirements-dev.txt` — Development Dependencies (~85 tok)
- `requirements-full.txt` — DevSkyy - Full ML/3D Dependencies (~46 tok)
- `requirements.txt` — Python dependencies (~109 tok)
- `run_generation.sh` — Set environment variables for Tripo3D generation (~104 tok)
- `skyyrose_clothing_barcodes.txt` (~459 tok)
- `vercel.json` — /*.ts": { (~525 tok)

## .claude/worktrees/inspiring-lewin/.claude/

- `IMMEDIATE_REQUIREMENTS.md` — Immediate Requirements - READ THIS FIRST (~499 tok)
- `mcp-config.json` (~116 tok)
- `ralph-loop-iteration-2.md` — Ralph Loop Iteration 2 - MCP Server Loading Fix (~601 tok)
- `settings.json` (~501 tok)
- `settings.local.json` (~339 tok)

## .claude/worktrees/inspiring-lewin/.claude/agents/

- `aidesigner-frontend.md` (~1481 tok)
- `architect.md` — Your Role (~1573 tok)
- `build-error-resolver.md` — Build Error Resolver (~936 tok)
- `code-reviewer.md` — Review Process (~2187 tok)
- `database-reviewer.md` — Database Reviewer (~1079 tok)
- `deploy-and-verify.md` — Deploy & Verify Agent (~325 tok)
- `doc-updater.md` — Documentation & Codemap Specialist (~831 tok)
- `e2e-runner.md` — E2E Test Runner (~1022 tok)
- `loop-operator.md` — Mission (~231 tok)
- `planner.md` — Your Role (~1763 tok)
- `python-reviewer.md` — Review Priorities (~843 tok)
- `refactor-cleaner.md` — Refactor & Dead Code Cleaner (~677 tok)
- `security-reviewer.md` — Security Reviewer (~1110 tok)
- `tdd-guide.md` — Your Role (~721 tok)
- `wp-code-simplifier.md` — WordPress Code Simplifier (~265 tok)

## .claude/worktrees/inspiring-lewin/.claude/commands/

- `aidesigner.md` (~799 tok)
- `build-fix.md` — Build and Fix (~143 tok)
- `checkpoint.md` — Checkpoint Command (~145 tok)
- `code-review.md` — Code Review (~247 tok)
- `deploy-wp.md` (~68 tok)
- `e2e.md` — E2E Command (~240 tok)
- `eval.md` — Eval Command (~167 tok)
- `fix-and-learn.md` (~101 tok)
- `learn.md` — /learn - Extract Reusable Patterns (~164 tok)
- `orchestrate.md` — Orchestrate Command (~227 tok)
- `plan-exit-review.md` — Plan Review Mode (~2659 tok)
- `plan.md` — Plan Command (~234 tok)
- `refactor-clean.md` — Refactor Clean (~180 tok)
- `tdd.md` — TDD Command (~266 tok)
- `test-coverage.md` — Test Coverage (~166 tok)
- `update-codemaps.md` — Update Codemaps (~176 tok)
- `update-docs.md` — Update Documentation (~183 tok)
- `verify.md` — Verification Command (~161 tok)
- `wp-simplify.md` (~46 tok)

## .claude/worktrees/inspiring-lewin/.claude/contexts/

- `dev.md` — Development Context (~105 tok)
- `research.md` — Research Context (~154 tok)
- `review.md` — Code Review Context (~132 tok)

## .claude/worktrees/inspiring-lewin/.claude/hooks/

- `hooks.json` — Declares check (~3245 tok)

## .claude/worktrees/inspiring-lewin/.claude/hooks/claude-md-tracker/

- `session-summary.sh` — Stop Hook — Summarize CLAUDE.md staleness at session end (~539 tok)
- `test-hooks.sh` — Test harness for CLAUDE.md staleness tracking hooks (~3208 tok)
- `track-changes.sh` — PostToolUse Hook — Track structural changes for CLAUDE.md staleness detection (~1534 tok)

## .claude/worktrees/inspiring-lewin/.claude/hooks/memory-persistence/

- `pre-compact.sh` — PreCompact Hook - Save state before context compaction (~312 tok)
- `session-end.sh` — Stop Hook (Session End) - Persist learnings when session ends (~409 tok)
- `session-start.sh` — SessionStart Hook - Load previous context on new session (~492 tok)

## .claude/worktrees/inspiring-lewin/.claude/hooks/strategic-compact/

- `suggest-compact.sh` — Strategic Compact Suggester (~467 tok)

## .claude/worktrees/inspiring-lewin/.claude/rules/

- `agents.md` — Agent Orchestration (~406 tok)
- `coding-style.md` — Coding Style (~350 tok)
- `development-workflow.md` — Development Workflow (~510 tok)
- `git-workflow.md` — Git Workflow (~156 tok)
- `hooks.md` — Hooks System (~192 tok)
- `patterns.md` — Common Patterns (~256 tok)
- `performance.md` — Performance Optimization (~400 tok)
- `prompt-techniques.md` — Prompt Technique Enhancement (Always Active) (~611 tok)
- `security.md` — Security Guidelines (~216 tok)
- `sequential-thinking.md` — Sequential Thinking (Auto-Trigger) (~329 tok)
- `testing.md` — Testing Requirements (~267 tok)

## .claude/worktrees/inspiring-lewin/.claude/skills/

- `code-review.md` — DevSkyy Code Review (~257 tok)
- `deploy.md` — DevSkyy Deployment Pipeline (~267 tok)
- `fix-linting.md` — DevSkyy Linting Auto-Fix (~232 tok)

## .claude/worktrees/inspiring-lewin/.claude/skills/aidesigner-frontend/

- `SKILL.md` (~1482 tok)

## .claude/worktrees/inspiring-lewin/.claude/skills/api-design/

- `SKILL.md` — API Design Patterns (~3271 tok)

## .claude/worktrees/inspiring-lewin/.claude/skills/api-design/agents/

- `openai.yaml` (~71 tok)

## .claude/worktrees/inspiring-lewin/__mocks__/

- `three.cjs` — Mock for three.js - used in Jest tests (~2964 tok)

## .claude/worktrees/inspiring-lewin/__mocks__/three/examples/jsm/controls/

- `OrbitControls.js.cjs` — Mock for three/examples/jsm/controls/OrbitControls.js (~163 tok)

## .claude/worktrees/inspiring-lewin/__mocks__/three/examples/jsm/postprocessing/

- `BokehPass.js.cjs` — Mock for three/examples/jsm/postprocessing/BokehPass.js (~129 tok)
- `EffectComposer.js.cjs` — Mock for three/examples/jsm/postprocessing/EffectComposer.js (~102 tok)
- `OutputPass.js.cjs` — Mock for three/examples/jsm/postprocessing/OutputPass.js (~63 tok)
- `RenderPass.js.cjs` — Mock for three/examples/jsm/postprocessing/RenderPass.js (~80 tok)
- `UnrealBloomPass.js.cjs` — Mock for three/examples/jsm/postprocessing/UnrealBloomPass.js (~111 tok)

## .husky/

- `pre-commit` — Pre-commit: lint, type-check, syntax, and test gates (~337 tok)

## __mocks__/

- `three.cjs` — Mock for three.js - used in Jest tests (~2964 tok)

## __mocks__/three/examples/jsm/controls/

- `OrbitControls.js.cjs` — Mock for three/examples/jsm/controls/OrbitControls.js (~163 tok)

## __mocks__/three/examples/jsm/postprocessing/

- `BokehPass.js.cjs` — Mock for three/examples/jsm/postprocessing/BokehPass.js (~129 tok)
- `EffectComposer.js.cjs` — Mock for three/examples/jsm/postprocessing/EffectComposer.js (~102 tok)
- `OutputPass.js.cjs` — Mock for three/examples/jsm/postprocessing/OutputPass.js (~63 tok)
- `RenderPass.js.cjs` — Mock for three/examples/jsm/postprocessing/RenderPass.js (~80 tok)
- `UnrealBloomPass.js.cjs` — Mock for three/examples/jsm/postprocessing/UnrealBloomPass.js (~111 tok)

## agents/

- `support_agent.py` — Declares SupportAgent (~9200 tok)

## api/v1/

- `elite_studio.py` — API: 8 endpoints (~4616 tok)
- `rag_anything.py` — API: 4 endpoints (~1987 tok)

## assets/brand/

- `brand.yaml` — /*.{php,py} or frontend/**/*.{ts,tsx,js,jsx} file. (~2555 tok)

## assets/product-masters/

- `manifest.json` (~17 tok)
- `README.md` — Project documentation (~771 tok)

## assets/prompts/

- `registry.yaml` — SkyyRose Prompt Library — SINGLE SOURCE OF TRUTH for prompts with production impact (~1402 tok)

## billing/

- `entitlements.py` — EntitlementResult: check, get_upgrade_message (~1530 tok)
- `middleware.py` — billing_middleware (~1538 tok)
- `plans.py` — TierLimits: get_limits, intent_allowed, quota_remaining (~1686 tok)

## core/middleware/

- `tenant.py` — tenant_middleware (~1186 tok)

## examples/

- `basic_query.py` — basic_example, single_agent_example, main (~620 tok)
- `continuous_conversation.py` — interactive_conversation, multi_turn_analysis, collaborative_workflow, main (~2249 tok)
- `multi_agent_workflow.py` — product_launch_workflow, campaign_optimization_workflow, customer_support_workflow, main (~1312 tok)

## frontend/app/admin/conversion/

- `page.tsx` — COLLECTION_CONFIG (~20296 tok)

## frontend/app/admin/social-media/

- `page.tsx` — PLATFORM_CONFIG (~9672 tok)

## frontend/app/api/social-media/generate/

- `route.ts` — Social Media Pipeline - Post Generation Route (~3948 tok)

## frontend/components/mascot/

- `MascotBubble.tsx` — COLLECTION_GREETINGS (~3509 tok)

## renders/

- `__init__.py` — SkyyRose renders pipeline — preflight verification + FASHN tryon orchestration. (~25 tok)

## scripts/

- `deploy-theme.sh` — scripts/deploy-theme.sh -- Production deploy script for SkyyRose WordPress theme (~7513 tok)
- `split_techflats.py` — Split composite techflat images into individual front/back views. (~2954 tok)
- `sync_brand_to_php.py` — Sync brand.yaml → wordpress-theme/skyyrose-flagship/inc/brand.generated.php. (~2406 tok)
- ~~`generate_catalog.py`~~ — RETIRED 2026-04-19, do not resurrect (canonical catalog is `wordpress-theme/skyyrose-flagship/data/skyyrose-catalog.csv`)
- ~~`sync_manifest_from_catalog.py`~~ — RETIRED 2026-04-19, do not resurrect

## sdk/python/agent_sdk/

- `custom_tools.py` — generate_3d_model, virtual_tryon, check_task_status, manage_product (~4124 tok)
- `main.py` — DevSkyy: main, execute, query_agent, round_table + 4 more (~1745 tok)
- `orchestrator.py` — AgentOrchestrator: get_orchestrator_options, execute_workflow, query_single_agent (~2261 tok)
- `round_table.py` — RoundTableOrchestrator: get_base_options, run_single_participant, run_round_table, execute_with_winn (~2504 tok)
- `worker.py` — BackgroundWorker: connect, process_generate_3d, process_fashn_tryon, process_task + 1 more (~4085 tok)

## sdk/python/agent_sdk/super_agents/

- `__init__.py` (~213 tok)

## services/

- `image_ingestion.py` — Image ingestion service for downloading and processing images. (~5342 tok)
- `rag_anything_service.py` — class: llm_model_func, initialize, ingest_document, query + 1 more (~4084 tok)

## skyyrose/elite_studio/

- `brand.py` — Brand — Single source of truth loader for SkyyRose brand identity. (~1564 tok)
- `catalog.py` — Catalog — Single source of truth loader for SkyyRose product data. (~4704 tok)
- `fidelity.py` — class: to_dict, check_color, check_text, check_clip_similarity + 1 more (~3254 tok)
- `master_registry.py` — MasterEntry: masters_dir, manifest_path, sha256_of_file, is_locked + 11 more (~3012 tok)
- `prompts.py` — Prompt Library — source of truth for prompts with production impact. (~3416 tok)
- `telemetry.py` — name: new_run_id, hash_inputs, emit, set + 1 more (~1277 tok)
- `validation.py` — Shared validation utilities for registry files. (~1381 tok)

## skyyrose/elite_studio/agents/

- `compositor_agent.py` — Logger: info, step, ok, fail + 2 more (~12690 tok)

## skyyrose/elite_studio/character/

- `agent.py` — CharacterCreationAgent: create_sheet, generate_pose, create_skyyrose_rosie (~4156 tok)

## skyyrose/elite_studio/graph/

- `builder.py` — GraphConfig: build_graph (~2673 tok)
- `nodes.py` — vision_node, generator_node, quality_node, human_review_node (~4968 tok)
- `runner.py` — run_single, run_batch (~1184 tok)
- `state.py` — EliteStudioState: create_initial_state, extract_production_result (~1019 tok)

## skyyrose/elite_studio/prompts/

- `__init__.py` (~361 tok)
- `cache.py` — PromptCache: check, store, invalidate_by_intent, get_stats (~3148 tok)
- `enhancer.py` — from: enhance, analyze_only (~1109 tok)
- `library.py` — Prompt Library — source of truth for prompts with production impact. (~3466 tok)

## skyyrose/elite_studio/queue/

- `consumer.py` — EliteStudioWorker: process_job, run_forever (~3208 tok)
- `producer.py` — enqueue_produce, aenqueue_produce, enqueue_creative, aenqueue_creative (~2010 tok)

## skyyrose/elite_studio/tests/

- `test_brand_enforcement.py` — Brand enforcement — retired taglines must never appear in generated content. (~1319 tok)
- `test_brand.py` — Tests for skyyrose.elite_studio.brand (BrandConfig SoT loader). (~1063 tok)
- `test_catalog_validation.py` — Tests for Catalog hardening (structural + referential integrity). (~2663 tok)
- `test_catalog.py` — Tests for skyyrose.elite_studio.catalog (Wave 1.5 SoT loader). (~2373 tok)
- `test_fidelity.py` — Tests for skyyrose.elite_studio.fidelity (Wave 1 fidelity gate). (~1869 tok)
- `test_master_registry.py` — Tests for skyyrose.elite_studio.master_registry (Wave 1 reference-first pipeline). (~1557 tok)
- `test_prompts.py` — Tests for skyyrose.elite_studio.prompts (PromptLibrary SoT + hardening). (~2522 tok)
- `test_telemetry.py` — Tests for skyyrose.elite_studio.telemetry (Phase 1 compositor instrumentation). (~1239 tok)
- `test_validation.py` — Tests for skyyrose.elite_studio.validation shared validators. (~1507 tok)

## skyyrose/multi_agent/

- `hooks.py` — audit_tool_use, guard_bash, log_file_edit, report_progress (~1457 tok)

## tasks/

- `HANDOFF-PR-453-P1S.md` — Handoff — PR #453 P1 Hotfix (`hotfix/pr-453-p1s`) (~2616 tok)
- `pr-453-action-report.md` — PR #453 Action Report (~1668 tok)
- `todo.md` — Current Tasks (~1455 tok)

## tests/

- `test_character_system.py` — TestCharacterSpec: test_create_basic_spec, test_spec_is_frozen, test_spec_default_embedding_path, te (~4718 tok)
- `test_hybrid_simple.py` — main (~1523 tok)
- `test_rag_integration.py` — rag_manager, ingested_rag_manager, test_rag_context_retrieval, test_agent_with_rag_context (~2267 tok)
- `test_saas_infrastructure.py` — immutability: test_frozen_dataclass_immutable, test_all_tiers_present, test_free_tier_renders_limit, (~8108 tok)
- `test_security.py` — TestAESGCMEncryption: test_encrypt_decrypt_string, test_encrypt_decrypt_bytes, test_encrypt_decrypt_ (~17535 tok)

## tests/integration/

- `test_hybrid_integration.py` — print_test, print_success, print_error, print_warning (~3579 tok)

## tests/wordpress/

- `test_collection_page_manager.py` — Tests for wordpress.collection_page_manager — verifies SSoT migration. (~966 tok)

## wordpress-theme/skyyrose-flagship/

- `404.php` — 404 Error Page — The Skyy Rose Collection (~4246 tok)
- `footer.php` — The template for displaying the footer (~3003 tok)
- `front-page.php` — Front Page — SkyyRose v7.0 "Concrete" Commercial Theme (~5164 tok)
- `functions.php` — SkyyRose Theme Functions (~1243 tok)
- `template-immersive-love-hurts.php` — Template Name: Immersive - Love Hurts (~725 tok)

## wordpress-theme/skyyrose-flagship/assets/css/

- `brand-atmosphere.css` — Brand Atmosphere — Canvas Overlay Styles (~369 tok)
- `commercial-polish.css` — SkyyRose Commercial Polish — premium presentation layer. (~3217 tok)
- `personalization.css` — Personalization — Curated For You Grid (~1188 tok)
- `skyy-walk.css` — Skyy — Pixar-Like Walk Cycle Enhancements (~1816 tok)
- `smart-showcase.css` — Smart Showcase — Quick-View Dialog Styles (~2218 tok)

## wordpress-theme/skyyrose-flagship/assets/js/

- `brand-atmosphere.js` — Brand Atmosphere — Canvas Particle System (~1978 tok)
- `experience-analyzer.js` — Experience Analyzer — Behavioral Tracking & Event Relay (~1921 tok)
- `mascot.min.js` — SkyyRose Brand Mascot — Skyy Living Character (~4000 tok)
- `performance-guardian.js` — Performance Guardian — FPS Watchdog & Animation Budget Manager (~951 tok)
- `personalization.js` — Personalization — Curated For You (~2026 tok)
- `skyy-3d.js` — Skyy — Three.js Walking Character (~2669 tok)
- `smart-showcase.js` — Smart Showcase — Quick-View Dialog (~3552 tok)

## wordpress-theme/skyyrose-flagship/inc/

- `admin-experience-dashboard.php` — Experience Engine — WordPress Admin Dashboard (~4686 tok)
- `enqueue.php` — Enqueue Scripts & Styles (~9710 tok)
- `performance-guardian.php` — Performance Guardian — Phase 2 (~848 tok)
- `personalization.php` — Personalization — Curated For You (~834 tok)
- `woocommerce.php` — WooCommerce Integration (~3982 tok)

## wordpress-theme/skyyrose-flagship/template-parts/

- `product-card-holo.php` — Template Part: Holo Product Card (~3569 tok)
- `skyy-mascot.php` — Skyy — Living Character Widget (~1207 tok)

## wordpress-theme/skyyrose-flagship/template-parts/components/

Phase 0 design system primitives. All PHPCS-clean (WP standard). No inline styles/scripts. BEM prefix: `sr-`.

- `button.php` — Button/anchor with variants (primary|ghost|outline|text), sizes (sm|md|lg), loading state (aria-busy + sr-btn__loading), SVG icon slots (~193 tok)
- `input.php` — Text/email/password/tel/url/search/number/textarea with inline error (aria-invalid + role=alert) and hint text (~151 tok)
- `form.php` — Form wrapper with optional nonce, aria-live status region, fieldset/legend, slot pattern (~116 tok)
- `figure.php` — Image with decorative/alt handling, srcset, aspect-ratio, object-fit variants; null alt = HTML comment flag (~125 tok)
- `card-product.php` — Product card via skyyrose_get_product($sku); early return if SKU missing; composes button.php (~144 tok)
- `card-editorial.php` — Full-bleed campaign card with kicker/headline/body/CTA; composes figure.php + button.php (~133 tok)
- `card-info.php` — Text-first info/stat card with icon slot, eyebrow, headline, body, CTA; schemes: default|elevated|bordered (~163 tok)
- `quote.php` — Blockquote with attribution, sizes (sm|md|lg|display), align, optional decorative glyph (~91 tok)
- `modal.php` — Accessible dialog (role=dialog, aria-modal, aria-labelledby, data-focus-trap); JS wired Phase 5 (~123 tok)
- `drawer.php` — Off-canvas panel (left|right|bottom), same ARIA pattern as modal; overlay dismiss, data-drawer-side (~140 tok)

## wordpress/

- `collection_page_manager.py` — CollectionType: to_dict, get_template, get_all_templates, to_agent_reference (~1439 tok)

## .claude/commands/

- `enhance.md` — /enhance slash command — Augment-style prompt rewriter with review/send/compare modes (~850 tok)

## .planning/handoffs/trellis2-deployment.md (~210 lines, ~3500 tokens)
Self-contained deployment handoff for Microsoft TRELLIS.2 (image-to-3D, 4B params, Linux+CUDA-only). Two-phase plan: Phase 1 = duplicate HF Space `microsoft/TRELLIS.2` to private `damBruh/skyyrose-trellis2` on A10G; Phase 2 = port to Modal serverless and wire `agents/trellis_agent.py` mirroring `agents/meshy_agent.py`. Includes cost matrix, pre-flight checklist, acceptance criteria, risks.
