...
class SkyyRoseWorkflowState(WorkflowState):
    ...
    # Analytics
    pricing_recommendation: Optional[float] = None
...
class WorkflowManager:
    ...
    def get_workflow(self, name: str) -> Optional[WorkflowGraph]:
...
def create_customer_support_workflow() -> WorkflowGraph:
    ...
    class RespondNode(AgentNode):
        async def execute(self, state: WorkflowState) -> WorkflowState:
            ...
            state.outputs[self.name] = {"response": responses.get(intent, "Thank you for contacting SkyyRose.")}
            return state
