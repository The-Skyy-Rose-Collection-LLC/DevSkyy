...
jest.mock('three', () => ({
  ...
  WebGLRenderer: jest.fn(() => ({ setSize: jest.fn(), setPixelRatio: jest.fn(), shadowMap: { enabled: false, type: 0 }, domElement: document.createElement('canvas'), render: jest.fn(), dispose: jest.fn(), forceContextLoss: jest.fn(), toneMapping: 0, toneMappingExposure: 1, outputColorSpace: '' })),
  Color: jest.fn(() => ({})),
  Fog: jest.fn(() => ({})),
  FogExp2: jest.fn(() => ({})),
  ...
  SpotLight: jest.fn(() => ({ position: { set: jest.fn() }, target: { position: { set: jest.fn() } }, castShadow: false, shadow: { mapSize: { width: 0, height: 0 } }, dispose: jest.fn() })),
