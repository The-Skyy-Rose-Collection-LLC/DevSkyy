...
const SocialMediaAutomation = () => {
  ...
  const fetchSocialData = async () => {
    try {
      setLoading(true)
      // Fetch campaign data
      const campaignsResponse = await axios.get(`${API_BASE_URL}/marketing/social-campaigns`)
      setCampaigns(campaignsResponse.data.campaigns || [])
      
      // Fetch platform connections
      const platformsResponse = await axios.get(`${API_BASE_URL}/integrations/social-platforms`)
  ...
  const connectPlatform = async (platform) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/integrations/social-connect`, {
        platform: platform,
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-gray-600 text-lg font-elegant max-w-4xl mx-auto">
          Automate your luxury streetwear brand across all social platforms with AI-powered content creation, scheduling, and engagement optimization.
        </p>
      </motion.div>
        ...
        {Object.entries(platforms).map(([platform, data]) => (
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-xl font-fashion font-semibold capitalize mb-2">{platform}</h3>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-fashion font-semibold text-gray-800">Active Campaigns</h3>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {campaigns.map((campaign) => (
            <div key={campaign.id} className="bg-gradient-to-r from-gray-50 to-white rounded-2xl p-6 border border-gray-200">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-xl font-semibold text-gray-800 mb-1">{campaign.name}</h4>
                  <div className="flex items-center space-x-2">
                    <span className="text-2xl">{platformIcons[campaign.platform]}</span>
                    <span className="capitalize text-gray-600">{campaign.platform}</span>
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-luxury-gold">${campaign.budget}</div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-800">{campaign.reach.toLocaleString()}</div>
                  <div className="text-sm text-gray-600">Reach</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-rose-gold">{campaign.engagement.toLocaleString()}</div>
                  <div className="text-sm text-gray-600">Engagement</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-emerald-600">{campaign.clicks.toLocaleString()}</div>
              ...
              <div className="mt-4 bg-gray-100 rounded-xl p-3">
                ...
                <div className="flex justify-between text-sm">
                  <span>CTR: {((campaign.clicks / campaign.reach) * 100).toFixed(2)}%</span>
                  <span>Engagement Rate: {((campaign.engagement / campaign.reach) * 100).toFixed(2)}%</span>
                </div>
              </div>
            </div>
          ))}
      ...
      >
        <h3 className="text-2xl font-fashion font-semibold text-gray-800 mb-6">Automation Rules</h3>
        
        <div className="space-y-4">
          {automationRules.map((rule) => (
            <div key={rule.id} className="bg-gradient-to-r from-gray-50 to-white rounded-2xl p-6 border border-gray-200">
              <div className="flex justify-between items-center">
                <div>
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">{rule.name}</h4>
                  <p className="text-gray-600 mb-3">{rule.trigger}</p>
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-2">
                      {rule.platforms.map((platform) => (
                        <span key={platform} className="text-lg">{platformIcons[platform]}</span>
                      ))}
                    </div>
      ...
      >
        <h3 className="text-2xl font-fashion font-semibold text-gray-800 mb-6">AI Content Studio</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-2xl p-6 shadow-elegant">
            ...
            <p className="text-gray-600 text-sm mb-4">AI-generated luxury streetwear visuals</p>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-elegant">
            ...
            <p className="text-gray-600 text-sm mb-4">Luxury brand captions and hashtags</p>
            <button className="bg-gradient-to-r from-rose-gold to-luxury-gold text-white px-4 py-2 rounded-xl text-sm font-semibold w-full">
              Generate Copy
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-elegant">
            ...
            <p className="text-gray-600 text-sm mb-4">Optimal posting times across platforms</p>
            <button className="bg-gradient-to-r from-rose-gold to-luxury-gold text-white px-4 py-2 rounded-xl text-sm font-semibold w-full">
              Schedule Posts
            </button>
          </div>
