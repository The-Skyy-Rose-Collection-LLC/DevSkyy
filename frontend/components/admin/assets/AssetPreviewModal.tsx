  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-white text-lg font-medium mb-4">{asset.name}</h2>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
          ...
          {asset.sku && (
            <div className="text-gray-400">SKU: <span className="text-white">{asset.sku}</span></div>
          )}
          <div className="text-gray-400">Uploaded: <span className="text-white">{new Date(asset.uploaded_at).toLocaleDateString()}</span></div>
        </div>
      </div>
    </div>
  );
