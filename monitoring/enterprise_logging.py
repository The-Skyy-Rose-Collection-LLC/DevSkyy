...
class EnterpriseLogger:
    ...
    def _create_log_entry(
        ...
        context = self.get_context() or LogContext(
            correlation_id=str(uuid.uuid4()), environment=os.getenv("ENVIRONMENT", "development")
        )
    ...
    def debug(
        self, message: str, category: LogCategory = LogCategory.SYSTEM, metadata: Optional[dict[str, Any]] = None
    ):
        """Log debug message."""
    ...
    def audit(self, action: str, resource: str, metadata: Optional[dict[str, Any]] = None):
        """Log audit event."""
        entry = self._create_log_entry(LogLevel.INFO, LogCategory.AUDIT, f"Audit: {action} on {resource}", metadata)
        self.audit_logger.info(entry)

    def security(self, event: str, severity: str = "medium", metadata: Optional[dict[str, Any]] = None):
        """Log security event."""
        security_metadata = {"severity": severity, "event_type": "security", **(metadata or {})}

        level = LogLevel.WARNING if severity in ["medium", "high"] else LogLevel.INFO
        entry = self._create_log_entry(level, LogCategory.SECURITY, f"Security Event: {event}", security_metadata)
    ...
    def performance(self, operation: str, duration: float, metadata: Optional[dict[str, Any]] = None):
...
class ConsoleFormatter(logging.Formatter):
    ...
    def format(self, record):
        if isinstance(record.msg, StructuredLogEntry):
            ...
            return f"{color}[{entry.timestamp}] {entry.level} " f"[{entry.category}] {entry.message}{reset}"
        else:
