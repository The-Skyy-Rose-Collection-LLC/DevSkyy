...
def get_clip() -> _ClipState:
    ...
    with _LOCK:
        ...
        model = (
            CLIPModel.from_pretrained(MODEL_ID).to(device).eval()
        )  # nosec B615 — openai/clip-vit-base-patch32 is a pinned well-known public model; revision pinned via MODEL_ID constant above
