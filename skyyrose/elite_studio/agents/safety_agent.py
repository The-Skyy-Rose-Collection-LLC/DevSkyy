"""SafetyAgent — Phase B2 stub (dual-agent rebuild pending).
...
Planned architecture (see .claude/plans/well-lets-audit-separately-humming-beacon.md):
    Agent A: Claude safety
    Agent B: Gemini safety
    Mode:    consensus
...
class SafetyAgent:
    ...
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            "SafetyAgent is a Phase B1 stub. See .claude/plans/"
            "well-lets-audit-separately-humming-beacon.md for the dual-agent "
            "rebuild design (consensus mode; Agent A=Claude safety, Agent B=Gemini safety)."
        )
