"""Smoke tests for the Editorial Photography venture."""
...
from skyyrose.elite_studio.ventures.photo import (
    MANIFEST,
    PHOTO_AGENTS,
    PhotoPipeline,
    build_pipeline,
...
def test_venture_registered() -> None:
    assert "photo" in list_ventures()
    assert get_manifest("photo") is MANIFEST
...
@pytest.mark.parametrize("role", ["vision", "renderer", "quality", "prompt", "standards_director"])
