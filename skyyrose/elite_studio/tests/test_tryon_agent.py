...
class TestCategoryMapping:
    ...
    def test_all_standard_categories_present(self) -> None:
        expected = {"upper_body", "tops", "lower_body", "bottoms", "dresses", "outerwear", "full_body"}
...
class TestTryOnSuccess:
    def test_returns_tryon_result_on_success(self, agent: TryOnAgent, tmp_path: Path) -> None:
        ...
        mock_fashn_result = {"image_path": str(fake_raw_path), "image_url": "https://example.com/img.png"}

        with (
            patch("skyyrose.elite_studio.agents.tryon_agent.OUTPUT_DIR", tmp_path),
            patch("skyyrose.elite_studio.agents.tryon_agent.asyncio.run", return_value=mock_fashn_result),
            patch("shutil.copy2"),
