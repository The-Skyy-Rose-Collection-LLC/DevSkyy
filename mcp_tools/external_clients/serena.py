...
Usage:
    from mcp_tools.external_clients.serena import serena_client
...
class SerenaClient:
    ...
    async def suggest_refactoring(
        ...
        Returns:
            List of refactoring suggestion dicts
        """
        result = await mcp_manager.call_tool(
            self.SERVER_ID,
            "suggest_refactoring",
    ...
    async def full_project_audit(
        ...
        """
        Run full project audit by analyzing every Python file under ``project_root``.

        Skips ``__pycache__``, ``.venv``, ``node_modules``, and ``.git`` directories.

        Args:
            project_root: Root directory of the project

        Returns:
            Comprehensive audit results with severity counts and per-file details
        """
            ...
            try:
                ...
                for issue in issues:
                    ...
                    if severity in results:
                        results[severity] += 1  # type: ignore[operator]
