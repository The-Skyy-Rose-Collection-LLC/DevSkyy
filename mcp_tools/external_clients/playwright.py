...
Usage:
    from mcp_tools.external_clients.playwright import playwright_client
...
from mcp_tools.external_clients.server_manager import mcp_manager
...
class PlaywrightMCPClient:
    ...
    async def screenshot(
        ...
        Returns:
            Screenshot image bytes (empty if failed)
        """
    ...
    async def evaluate(self, script: str) -> Any:
        ...
        Returns:
            Script result or None on failure
        """
        result = await mcp_manager.call_tool(
            self.SERVER_ID,
            "browser_evaluate",
    ...
    async def test_3d_viewer(self, product_url: str) -> dict[str, Any]:
        ...
        1. 3D viewer button exists
        2. Modal opens on click
        3. Canvas rendered
        4. Loading completes
        5. WebGL context available

        Args:
            product_url: URL of the product page

        Returns:
            Test results dict with ``tests_passed``, ``tests_failed``, and ``details``
        """
        ...
        # Take screenshot (best-effort; failure does not affect test counts)
        try:
    ...
    async def test_wordpress_page(self, page_url: str) -> dict[str, Any]:
        ...
        Runs 3 checks:
        1. Page loads (has a title)
        2. No JavaScript errors
        3. Elementor content present
