...
No network calls are made at import time — connections are lazy and
created only when a tool method is first invoked.
"""
    ...
    async def connect(self, server_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Server might not have a health endpoint; proceed anyway
                self._clients[server_id] = client
                self._status[server_id] = MCPServerStatus.CONNECTED
...
# Singleton instance — shared by all client modules in this package
