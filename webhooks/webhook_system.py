...
class WebhookManager:
    ...
    async def emit(self, event_type: WebhookEvent, data: Dict[str, Any]):
        ...
        Args:
            ...
            data: Event data
        """
        payload = WebhookPayload(event_type=event_type, timestamp=datetime.now(timezone.utc), data=data)
    ...
    async def _deliver_webhook(self, subscription: WebhookSubscription, payload: WebhookPayload, attempt: int = 1):
        ...
        try:
            ...
            if attempt < subscription.max_retries:
                ...
                logger.error(
                    f"Webhook permanently failed after {attempt} attempts: " f"{subscription.endpoint} - {str(e)}"
                )
