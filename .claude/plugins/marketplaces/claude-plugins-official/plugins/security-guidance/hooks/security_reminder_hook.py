        ...
        if "substrings" in pattern and content:
            for substring in pattern["substrings"]:
                if substring in content:
                    # Special case: Skip PyTorch .eval() method (model.eval(), self.model.eval(), etc.)
                    if substring == "eval(" and ".eval(" in content:
                        # Check if all occurrences are preceded by a dot (method call)
                        import re

                        eval_calls = re.finditer(r"\beval\(", content)
                        has_non_method_eval = False
                        for match in eval_calls:
                            start = match.start()
                            # Check character before 'eval('
                            if start > 0 and content[start - 1] != ".":
                                has_non_method_eval = True
                                break
                        if not has_non_method_eval:
                            continue  # All eval( calls are .eval() methods, skip warning
