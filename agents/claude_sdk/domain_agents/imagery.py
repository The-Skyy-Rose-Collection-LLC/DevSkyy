...
class SDKVirtualTryOnAgent(SDKSubAgent):
    ...
    def _sdk_default_prompt(self) -> str:
        return (
            ...
            "After writing a render, update the catalog CSV's front_model_image "
            "or back_model_image column for this SKU to point at the new file, "
            "then regenerate sot.json (build-collection-sot.py) — other systems "
            "(dashboard, theme) discover renders via "
            "skyyrose.core.sot_images.resolve_image(), not filename convention.\n\n"
