...
class SDKVirtualTryOnAgent(SDKSubAgent):
    ...
    def _sdk_default_prompt(self) -> str:
        return (
            ...
            "After writing a front or back render, update the catalog CSV's "
            "front_model_image or back_model_image column for this SKU to point "
            "at the new file. Branding renders are supplementary assets with no "
            "SOT role — NEVER register one in those columns. Then regenerate the "
            "per-collection sot.json files (build-collection-sot.py) — that is "
            "what skyyrose.core.sot_images.resolve_image() reads — and refresh "
            "the downstream manifest data/sot-images.json (make sot-manifest) "
            "for the dashboard/theme. Renders are discovered via the SOT, not "
            "filename convention.\n\n"
