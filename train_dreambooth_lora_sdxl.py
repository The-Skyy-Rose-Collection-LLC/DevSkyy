...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--prodigy_decouple", type=bool, default=True, help="Use AdamW style decoupled weight decay"
    )
    parser.add_argument(
        "--adam_weight_decay", type=float, default=1e-04, help="Weight decay to use for unet params"
    )
    parser.add_argument(
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        interpolation = getattr(
            transforms.InterpolationMode, args.image_interpolation_mode.upper(), None
        )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if latents_mean is None and latents_std is None:
                    ...
                    model_input = (
                        (model_input - latents_mean) * vae.config.scaling_factor / latents_std
                    )
