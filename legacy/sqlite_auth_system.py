...
class SQLiteAuthSystem:
    ...
    def __init__(self, db_config: DatabaseConfig = None, security_config: SecurityConfig = None):
    ...
    def authenticate_user(
        ...
        try:
            ...
            if not self.verify_password(password, user[3], user[4]):
                ...
                if failed_attempts >= self.security_config.max_login_attempts:
                    ...
                    self._log_action(
                        ...
                        user[0],
                        "account_locked",
                        {"reason": "max_login_attempts", "ip": ip_address},
                    )
            ...
            # Create session
            session_token = self.create_session(conn, user[0], ip_address, user_agent)
    ...
    def validate_session(self, session_token: str) -> tuple[bool, int | None]:
        """
    ...
    def generate_jwt_token(self, user_id: int, username: str, is_admin: bool) -> str:
        """Generate JWT token"""
        payload = {
            "user_id": user_id,
            "username": username,
    ...
    def verify_jwt_token(self, token: str) -> tuple[bool, dict[str, Any] | None]:
        """
        Verify JWT token
    ...
    def get_audit_logs(self, user_id: int | None = None, limit: int = 100) -> list[dict[str, Any]]:
        """Get audit logs"""
        conn = sqlite3.connect(self.db_config.db_path)
        cursor = conn.cursor()
...
def main():
    ...
    success, message, _ = auth.authenticate_user(
        username="john_doe", password="WrongPassword", ip_address="192.168.1.1"
    )
    print(f"   Result: {message}")
...
async def async_example():
    ...
    success, message, user_data = await auth.authenticate_user_async(
        username="async_user", password="AsyncPass123!"
    )
