...
class MediaManager:
    ...
    async def upload_3d_model(
        ...
        if product_id and item.source_url:
            try:
                ...
                logger.info(
                    f"3D model uploaded. Update product {product_id} with URL: {item.source_url}"
                )
    ...
    async def bulk_upload(
        ...
        tasks = [upload_with_limit(path, i) for i, path in enumerate(file_paths)]
    ...
    async def search(self, query: str, per_page: int = 20) -> builtins.list[MediaItem]:
        """Search media by query"""
        return await self.list(search=query, per_page=per_page)

    async def get_images(self, per_page: int = 20) -> builtins.list[MediaItem]:
        """Get only images"""
        return await self.list(media_type=MediaType.IMAGE, per_page=per_page)

    async def get_3d_models(self, per_page: int = 20) -> builtins.list[MediaItem]:
        """Get 3D model files"""
    ...
    async def get_by_post(self, post_id: int, per_page: int = 100) -> builtins.list[MediaItem]:
