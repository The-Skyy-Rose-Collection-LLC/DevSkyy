...
""")
        ...
        result = subprocess.run(
            [
                ...
                str(test_file),
                '--cov=.',
                '--cov-report=xml:coverage-test1.xml',
                '-v'
            ],
            cwd=str(tmp_path),
            env=env,
...
""")
        ...
        result = subprocess.run(
            [
                'coverage', 'combine',
                str(tmp_path / '.coverage.test1'),
                str(tmp_path / '.coverage.test2')
            ],
            cwd=str(tmp_path),
            capture_output=True,
            text=True
