...
def main():
    ...
    def t_chat():
        res = client.generate_content('Say "Grok connected" — nothing else.', model="grok-3-mini")
        assert "grok" in res["text"].lower() or "connected" in res["text"].lower()

    def t_streaming():
        chunks = list(
            client.generate_content_stream("Count to 3.", model="grok-3-mini", max_tokens=20)
        )
