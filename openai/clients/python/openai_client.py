...
class OpenAIClient:
    ...
    def start_chat(
        self, system_prompt: str = "", history: list[dict] | None = None, model: str | None = None
    ):
    ...
    def generate_with_tools(
        ...
        try:
            ...
            return {"text": msg.content or "", "tool_call": tool_call, "usage": response.usage}
        except Exception as e:
            return self._handle_error(e)

    def embed_content(self, text, model: str = "text-embedding-3-small"):
    ...
    def transcribe_audio(
        self, file_path: str, model: str = "whisper-1", language: str | None = None
    ) -> dict[str, Any]:
    ...
    def _handle_error(self, error: Exception) -> dict[str, Any]:
        ...
        if "authentication" in msg or "api key" in msg or "401" in msg:
            response.update(
                type="AuthenticationError", message="Invalid API key. Check OPENAI_API_KEY"
            )
        elif "rate limit" in msg or "429" in msg:
            response.update(
                type="RateLimitError", message="Rate limit exceeded. Please wait and retry"
            )
        elif "400" in msg:
