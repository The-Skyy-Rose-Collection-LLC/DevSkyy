...
.PHONY: help install dev-install clean test lint format docker-build docker-run setup quality style security all
# Aggregate target
all: check
...
lint:
	...
	black --check $(SRC_DIRS) $(TEST_DIRS) || true
	@echo "Checking import order..."
	isort --check-only $(SRC_DIRS) $(TEST_DIRS) || true
...
frontend-install:
	@if [ -d frontend ]; then \
		...
		echo "No frontend directory detected; skipping npm install"; \
	fi
...
frontend-build:
	@if [ -d frontend ]; then \
		...
		echo "No frontend directory detected; skipping frontend build"; \
	fi
...
frontend-dev:
	@if [ -d frontend ]; then \
		...
		echo "No frontend directory detected; skipping frontend dev server"; \
	fi
...
frontend-lint:
	@if [ -d frontend ]; then \
		...
		echo "No frontend directory detected; skipping frontend lint"; \
	fi
...
frontend-format:
	@if [ -d frontend ]; then \
		...
		echo "No frontend directory detected; skipping frontend format"; \
	fi
